...
export function generateCave(
  ...
  const { contents: matrixContents, width: newWidth } = surround(
    materials,
    width,
  );
  ...
  let tiles: Tile[] = materials.map((m, i) => {
    ...
    return {
      image,
      x: dimentions.x * config.OBSTACLE_SIZE,
      y: dimentions.y * config.OBSTACLE_SIZE,
    };
  });
...
function stampLiquids(
  ...
  make_pool: for (let i = 0; i < NUMBER_OF_POOLS; i++) {
    // Prevent infinite loop if it cannot find a new place for a pool
    if (failedAttempts > 100) {
      return;
    }
    const stamp =
      LiquidPools[randInt(0, LiquidPools.length, underworld.random)];
    // Start the stamp
    ...
    if (stamp) {
      const stampRecord = {
        start: startStampPosition,
          ...
          startStampPosition,
          oneDimentionIndexToVec2(stamp.contents.length - 1, stamp.width),
        ),
      };
...
function makeLevelMaterialsArrayRoomStyle(
  params: CaveParams,
  underworld: Underworld,
) {
  let width = 64;
  ...
  let lastStamp:
    | { position: Vec.Vec2; width: number; height: number }
    | undefined;
  for (let i = 0; i < NUMBER_OF_HALLS; i++) {
    ...
    let stamp: Material[] = Array(stampWidth * stampHeight).fill(
      Material.GROUND,
    );
    let stampPosition: Vec.Vec2 =
      lastStamp !== undefined
        ? // Ensure all stamps after the first stamp are touching each other so you get one cohesive map
          {
            x: randInt(
              lastStamp.position.x,
              lastStamp.position.x + lastStamp.width,
              underworld.random,
            ),
            y: randInt(
              lastStamp.position.y,
              lastStamp.position.y + lastStamp.height,
              underworld.random,
            ),
          }
        : // First stamp can be put anywhere
          {
            x: randInt(0, width / 2 - 1 - stampWidth, underworld.random),
            y: randInt(0, height / 2 - 1 - stampHeight, underworld.random),
          };
    lastStamp = {
...
function makeLevelMaterialsArrayCaveStyle(
  params: CaveParams,
  underworld: Underworld,
) {
  // Debug: Draw caves
  ...
  for (let c = 0; c < NUMBER_OF_CRAWLERS - 1; c++) {
    ...
    const unsignedStartPosition = Vec.round(
      Vec.random(
        params.startPointJitter * 0.75,
        params.startPointJitter,
        underworld.random,
      ),
    );
    ...
    crawl(
      cc,
      previousCrawler
        ? (previousCrawler.path[1] as Vec.Vec2)
        : Vec.round(
            Vec.random(
              -params.startPointJitter,
              params.startPointJitter,
  ...
  // Get bounds
  const crawlerBounds = getLimits(
    crawlers.map((c) => [...c.left, ...c.right]).flat(),
  );
  ...
  let width =
    Math.ceil(
      (crawlerBounds.xMax - crawlerBounds.xMin) / config.OBSTACLE_SIZE,
    ) + 2;
  let height =
    Math.ceil(
      (crawlerBounds.yMax - crawlerBounds.yMin) / config.OBSTACLE_SIZE,
    ) + 2;
  let materials: Material[] = Array(width * height).fill(Material.EMPTY);
  // Normalize crawlers to 0,0 in upper left corner
  function normalizeTo00(points: Vec.Vec2[]): Vec.Vec2[] {
    return points.map((p) => ({
      x: p.x - crawlerBounds.xMin,
      y: p.y - crawlerBounds.yMin,
    }));
  }
...
export function convertBaseTilesToFinalTiles(map: Map) {
  ...
  for (let i = 0; i < size; i++) {
    ...
    const neighbors = (
      Object.keys(SIDES_WITH_DIAG) as SIDES_WITH_DIAG_KEYS[]
    ).reduce<Record<SIDES_WITH_DIAG_KEYS, string>>(
      (neighbors, side) => {
    ...
    if (currentCell?.image == baseTiles.ground) {
      ...
      if (tile) {
        if (
          neighbors.west == baseTiles.liquid &&
          (neighbors.south == baseTiles.liquid ||
            neighbors.southeast == baseTiles.liquid)
        ) {
          changeTile(i, finalTileImages.liquidInsideCornerNE);
          map.liquid.push({
            image: finalTileImages.all_liquid,
            x: tile.x,
            y: tile.y,
          });
        } else if (
          neighbors.east == baseTiles.liquid &&
          (neighbors.south == baseTiles.liquid ||
            neighbors.southwest == baseTiles.liquid)
        ) {
          ...
          neighbors.east == baseTiles.liquid &&
          (neighbors.north == baseTiles.liquid ||
            neighbors.northwest == baseTiles.liquid)
        ) {
          ...
          neighbors.west == baseTiles.liquid &&
          (neighbors.north == baseTiles.liquid ||
            neighbors.northeast == baseTiles.liquid)
        ) {
...
function crawlersChangeTilesToMaterial(
  crawlers: CaveCrawler[],
  material: Material,
  caveWidth: number,
  caveHeight: number,
  caveMaterialsArray: Material[],
) {
    ...
    for (let y = 0; y < caveHeight; y++) {
      ...
      for (let crawler of crawlers) {
        for (let rect of crawler.rectangles) {
          if (
            isVec2InsidePolygon(
              { x: x * config.OBSTACLE_SIZE, y: y * config.OBSTACLE_SIZE },
              rect,
            )
          ) {
            isInside = true;
...
function movePointInDirection(
  ...
  cc.position = Vec.round(
    Vec.getEndpointOfMagnitudeAlongVector(cc.position, cc.direction, velocity),
  );
}
function crawl(
  cc: CaveCrawler,
  endPosition: Vec.Vec2,
  params: CaveParams,
  underworld: Underworld,
) {
  ...
  for (let i = 0; i < params.iterations; i++) {
    const turnRadians = randFloat(
      -directionRandomAmount,
      directionRandomAmount,
      underworld.random,
    );
  ...
  if (endPosition) {
    ...
    while (
      distance(cc.position, endPosition) >
      params.velocity + params.velocity * 0.25
    ) {
      cc.position = Vec.round(
        Vec.add(
          cc.position,
          similarTriangles(
            endPosition.x - cc.position.x,
            endPosition.y - cc.position.y,
            distance(cc.position, endPosition),
            params.velocity,
          ),
        ),
      ...
      cc.position = Vec.round(
        Vec.jitter(cc.position, params.velocity / 2, underworld.random),
      );
      cc.path.push(cc.position);
    }
    ...
    for (let i = 0; i < cc.path.length; i++) {
      ...
      const left = {
        x: p.x + Math.cos(direction + Math.PI / 2),
        y: p.y + Math.sin(direction + Math.PI / 2),
      };
      const right = {
        x: p.x + Math.cos(direction - Math.PI / 2),
        y: p.y + Math.sin(direction - Math.PI / 2),
      ...
      let newLeft = Vec.add(
        p,
        similarTriangles(left.x - p.x, left.y - p.y, tangentDist, cc.thickness),
      );
      // Jitter the left and right sides so they are not perfectly parallel
      newLeft = Vec.round(
        Vec.jitter(newLeft, cc.thickness / 4, underworld.random),
      );
      cc.left.push(newLeft);
      let newRight = Vec.add(
        p,
        similarTriangles(
          right.x - p.x,
          right.y - p.y,
      ...
      // Jitter the left and right sides so they are not perfectly parallel
      newRight = Vec.round(
        Vec.jitter(newRight, cc.thickness / 4, underworld.random),
      );
      ...
      if (lastLeft && lastRight) {
        ...
        if (
          lineSegmentIntersection(
            { p1: lastLeft, p2: newLeft },
            { p1: newRight, p2: lastRight },
          )
        ) {
          ...
          lineSegmentIntersection(
            { p1: newLeft, p2: newRight },
            { p1: lastLeft, p2: lastRight },
          )
        ) {
  ...
  if (t.image == finalTileImages.wall) {
    return {
      ...
      bounds: [
        ...
        .reverse()
        .map(({ x, y }) => ({
          x: x + t.x - config.OBSTACLE_SIZE / 2,
          y: y + t.y - config.OBSTACLE_SIZE / 2,
        })),
    };

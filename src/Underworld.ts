...
import {
  ...
  LineSegment,
  findWherePointIntersectLineSegmentAtRightAngle,
  closestLineSegmentIntersection,
} from './jmath/lineSegment';
import {
  expandPolygon,
  isVec2InsidePolygon,
  mergePolygon2s,
...
import {
  collideWithLineSegments,
  ForceMove,
  forceMovePreventForceThroughWall,
  isVecIntersectingVecWithCustomRadius,
  moveWithCollisions,
} from './jmath/moveWithCollision';
...
import {
  oneDimentionIndexToVec2,
  vec2ToOneDimentionIndexPreventWrap,
} from './jmath/ArrayUtil';
...
import {
  ...
  showPerkList,
  hidePerkList,
  createCursePerkElement,
  StatCalamity,
  generateRandomStatCalamity,
} from './Perk';
...
const elUpgradePicker = document.getElementById('upgrade-picker') as
  | HTMLElement
  | undefined;
export const elUpgradePickerContent = document.getElementById(
  'upgrade-picker-content',
) as HTMLElement | undefined;
const elSeed = document.getElementById('seed') as HTMLElement | undefined;
const elUpgradePickerLabel = document.getElementById('upgrade-picker-label') as
  | HTMLElement
  | undefined;
...
export default class Underworld {
  ...
  constructor(
    ...
    pie: PieClient | IHostApp,
    seed: string,
    RNGState: SeedrandomState | boolean = true,
  ) {
    ...
    cleanupRegistry?.register(
      this,
      `underworld-${this.seed}-${this.localUnderworldNumber}`,
    );
  ...
  getPotentialTargets(prediction: boolean): HasSpace[] {
    if (prediction) {
      return [
        ...this.unitsPrediction.filter((u) => !u.flaggedForRemoval),
        ...this.pickupsPrediction.filter((p) => !p.flaggedForRemoval),
        ...this.doodadsPrediction,
      ];
    } else {
      return [
        ...this.units.filter((u) => !u.flaggedForRemoval),
        ...this.pickups.filter((p) => !p.flaggedForRemoval),
        ...this.doodads,
      ];
    }
  }
  calculateKillsNeededForLevel(level: number): number {
    ...
    for (
      let i = startNumberOfEnemiesNeededToDrop;
      i < level + startNumberOfEnemiesNeededToDrop;
      i++
    ) {
  ...
  reportEnemyKilled(enemyKilledPos: Vec2) {
    ...
    const numberOfEnemiesKilledNeededForNextDrop =
      this.getNumberOfEnemyKillsNeededForNextLevelUp();
    if (numberOfEnemiesKilledNeededForNextDrop <= this.enemiesKilled) {
      ...
      if (
        Object.values(Cards.allCards).filter((c) => c.probability > 0).length >
  ...
  syncPlayerPredictionUnitOnly() {
    if (this.unitsPrediction && globalThis.player !== undefined) {
      const predictionUnit = this.unitsPrediction.find(
      ...
      if (predictionUnit) {
        ...
        Object.assign(
          predictionUnit,
          Unit.copyForPredictionUnit(globalThis.player.unit, this),
        );
      }
    }
  }
  ...
  syncPredictionEntities() {
    ...
    this.unitsPrediction = this.units.map((u) =>
      Unit.copyForPredictionUnit(u, this),
    );
  ...
  fullySimulateForceMovePredictions() {
    ...
    while (
      this.forceMovePrediction.length > 0 &&
      loopCount < PREVENT_INFINITE_WITH_WARN_LOOP_THRESHOLD
    ) {
      loopCount++;
        ...
        if (forceMoveInst) {
          ...
          if (globalThis.predictionGraphics && !globalThis.isHUDHidden) {
            ...
            globalThis.predictionGraphics.lineTo(
              forceMoveInst.pushedObject.x,
              forceMoveInst.pushedObject.y,
            );
  ...
  runForceMove(forceMoveInst: ForceMove, prediction: boolean): boolean {
    const { pushedObject, velocity, velocity_falloff, timedOut } =
      forceMoveInst;
    if (timedOut) {
      return true;
    }
    ...
    const aliveUnits = (
      prediction && this.unitsPrediction ? this.unitsPrediction : this.units
    ).filter((u) => u.alive);
    ...
    if (handled) {
      ...
      if (Unit.isUnit(pushedObject)) {
        ...
        Unit.takeDamage(
          pushedObject,
          damage,
          Vec.add(pushedObject, { x: velocity.x, y: velocity.y }),
          this,
          prediction,
        );
        if (!prediction) {
    ...
    for (let other of aliveUnits) {
      ...
      // recursion
      if (
        forceMoveInst.canCreateSecondOrderPushes &&
        isVecIntersectingVecWithCustomRadius(
          pushedObject,
          other,
    ...
    if (Unit.isUnit(forceMoveInst.pushedObject)) {
      ...
      // as it is pushed
      (prediction && this.unitsPrediction
        ? this.unitsPrediction
        : this.units
      ).forEach((u) => {
  ...
  gameLoopForceMove = () => {
    ...
    // to prevent slowdown
    const amountOfBloodGeometryPoints =
      graphicsBloodSmear?.geometry.points.length || 0;
    ...
    for (let i = this.forceMove.length - 1; i >= 0; i--) {
      ...
      if (forceMoveInst) {
        ...
        const endPos = {
          x: forceMoveInst.pushedObject.x,
          y: forceMoveInst.pushedObject.y + unitImageYOffset,
        };
        if (
          graphicsBloodSmear &&
          Unit.isUnit(forceMoveInst.pushedObject) &&
          forceMoveInst.pushedObject.health !== undefined &&
          forceMoveInst.pushedObject.health <= 0
        ) {
          const size = 3;
          for (let j of smearJitter) {
            // Multiple blood trails
            graphicsBloodSmear.beginFill(
              forceMoveInst.pushedObject.bloodColor,
              1.0,
            );
            ...
            if (
              !this.isInsideLiquid(startPos) &&
              !this.isInsideLiquid(endWithJitter)
            ) {
    ...
    if (finishedForceMoves) {
      ...
      if (forceMoveResolver) {
        ...
        console.error(
          'Unexpected: Finished forceMoves but forceMoveResolver is undefined',
        );
      }
  ...
  gameLoopUnit = (
    ...
    if (u.alive) {
      while (
        u.path &&
        u.path.points[0] &&
        Vec.equal(Vec.round(u), Vec.round(u.path.points[0]))
      ) {
      ...
      if (u.path && u.path.points[0] && u.stamina > 0 && isUnitsTurn) {
        // Move towards target
        const stepTowardsTarget = math.getCoordsAtDistanceTowardsTarget(
          u,
          u.path.points[0],
          u.moveSpeed * deltaTime,
        );
        let moveDist = 0;
        ...
        if (
          ...
          u.unitSubType == UnitSubType.MELEE &&
          math.distance(u, u.path.points[0]) <= config.COLLISION_MESH_RADIUS * 2
        ) {
    ...
    if (globalThis.headless) {
      ...
      while (moreProcessingToBeDone) {
        ...
        if (loopCount >= 1000 && loopCount % 2000 == 0) {
          console.error(
            'Headless gameloop unexpectedly large loop count:',
            loopCount,
          );
        }
        if (loopCount > 10000) {
          ...
          console.error(
            'Force return from headless gameloop to prevent infinite loop',
          );
          return;
        }
      }
      console.log(
        'Headless server executed gameloop in ',
        (performance.now() - headlessGameLoopPerfStart).toFixed(2),
        ' millis with',
        loopCount,
        ' loops.',
      );
  ...
  _gameLoopHeadless = (): boolean => {
    ...
    const aliveNPCs = this.units.filter(
      (u) => u.alive && u.unitType == UnitType.AI,
    );
  ...
  gameLoop = (timestamp: number) => {
    ...
    for (let i = 0; i < this.units.length; i++) {
      ...
      if (u) {
        ...
        if (
          (u.modifiers[shield.id] ||
            u.modifiers[fortify.id] ||
            u.modifiers[immune.id]) &&
          u.image
        ) {
    ...
    this.particleFollowers = this.particleFollowers.filter(
      (pf) => pf.emitter && !pf.emitter.destroyed,
    );
    ...
    if (
      ...
      timemasons.length &&
      globalThis.view == View.Game
    ) {
      timemasons.forEach((timemason) => {
        if (
          timemason.isSpawned &&
          timemason.unit.alive &&
          timemason.unit.mana > 0
        ) {
          let drainPerSecond =
            (timemason.unit.manaMax * config.TIMEMASON_PERCENT_DRAIN) / 100;
    ...
    this.aquirePickupQueue = this.aquirePickupQueue.filter(
      (p) => !p.flaggedForRemoval,
    );
    const now = Date.now();
      ...
      if (queuedPickup.timeout <= now) {
        ...
        if (pickup) {
          if (unit) {
            ...
            console.error(
              'Attempted to aquire queued pickup via timeout but unit is undefined',
            );
          ...
          console.error(
            'Attempted to aquire queued pickup via timeout but pickup is undefined',
          );
          // Prevent error from triggering more than once
          queuedPickup.flaggedForRemoval = true;
        }
  ...
  calculatePath(
    preExistingPath: Unit.UnitPath | undefined,
    startPoint: Vec2,
    target: Vec2,
  ): Unit.UnitPath {
  ...
  calculatePathNoCache(startPoint: Vec2, target: Vec2): Unit.UnitPath {
    ...
    if (points.length == 0) {
      ...
      for (let wall of this.pathingLineSegments) {
        const intersection = findWherePointIntersectLineSegmentAtRightAngle(
          target,
          wall,
        );
        if (intersection) {
      ...
      if (nearPointsOnWalls[0]) {
        const closest = nearPointsOnWalls.reduce<{
          intersection: Vec2;
          dist: number;
        }>(
          (acc, cur) => {
  ...
  drawPlayerThoughts() {
    ...
    function getXLocationOfImageForThoughtBubble(
      originX: number,
      index: number,
      totalNumberOfSpells: number,
    ) {
      return (
        originX + (0.5 + index - totalNumberOfSpells / 2) * spaceBetweenIcons
      );
    }
    ...
    for (let [thinkerClientId, thought] of Object.entries(
      this.playerThoughts,
    )) {
      const { target, cardIds, ellipsis } = thought;
      const thinkingPlayerIndex = this.players.findIndex(
        (p) => p.clientId == thinkerClientId,
      );
      ...
      if (thinkingPlayer && thinkingPlayer.isSpawned) {
        ...
        const yMargin = 10;
        const y =
          thinkingPlayer.unit.y - config.COLLISION_MESH_RADIUS * 2 - yMargin;
        ...
        for (let i = 0; i < cardIds.length; i++) {
          ...
          if (card) {
            const x = getXLocationOfImageForThoughtBubble(
              thinkingPlayer.unit.x,
              i,
              cardIds.length,
            );
        ...
        if (firstCard && lastCard) {
          ...
          const cardSize = 12;
          globalThis.thinkingPlayerGraphics?.drawRoundedRect(
            firstCard.x - cardSize,
            firstCard.y - cardSize,
            lastCard.x - firstCard.x + cardSize * 2,
            lastCard.y - firstCard.y + cardSize * 2,
            2,
          );
          globalThis.thinkingPlayerGraphics?.endFill();
        }
        thought.currentDrawLocation = Vec.lerpVec2(
          thought.currentDrawLocation || target,
          target,
          thought.lerp++ / 100,
        );
        ...
        if (currentDrawLocation && cardIds.length) {
          ...
          globalThis.thinkingPlayerGraphics?.lineStyle(
            3,
            colors.healthAllyGreen,
            1.0,
          );
  ...
  cacheWalls(obstacles: Obstacle.IObstacle[], emptyTiles: Tile[]) {
    const distanceFromTileCenterWhenAdjacent =
      1 + (Math.sqrt(2) * config.OBSTACLE_SIZE) / 2;
    ...
    function filterRemoveEmptyTileAdjacentPoly(poly: Polygon2): boolean {
      return !emptyTiles.some((tile) =>
        poly.some(
          (p) => math.distance(tile, p) <= distanceFromTileCenterWhenAdjacent,
        ),
      );
    }
    const getWallPolygons = () =>
      obstacles.filter((o) => o.material == Material.WALL).map((o) => o.bounds);
    ...
    this.pathingPolygons = mergePolygon2s([
      ...getWallPolygons()
        .map((p) => expandPolygon(p, config.COLLISION_MESH_RADIUS * 0.4))
    ...
    this.pathingLineSegments = this.pathingPolygons
      .map(toPolygon2LineSegments)
      .flat();
  }
  spawnPickup(index: number, coords: Vec2, prediction?: boolean) {
    ...
    if (pickup) {
      Pickup.create(
        { pos: coords, pickupSource: pickup, logSource: 'spawnPickup' },
        this,
        !!prediction,
      );
    } else {
  ...
  spawnEnemy(id: string, coords: Vec2, isMiniboss: boolean) {
    ...
    if (!sourceUnit) {
      console.error(
        'Unit with id',
        id,
        'does not exist.  Have you registered it in src/units/index.ts?',
      );
      return;
    }
    if (
      globalThis.enemyEncountered &&
      !globalThis.enemyEncountered.includes(id)
    ) {
      globalThis.enemyEncountered.push(id);
      storage.set(
        storage.ENEMY_ENCOUNTERED_STORAGE_KEY,
        JSON.stringify(globalThis.enemyEncountered),
      );
      ...
      setTimeout(() => {
        Jprompt({
          imageSrc: Unit.getExplainPathForUnitId(id),
          text: `<h1>${id}</h1>` + '\n' + i18n(sourceUnit.info.description),
          yesText: 'Okay',
        });
      }, 500);
    }
  ...
  testLevelData(): LevelData {
    ...
    const _tiles: Tile[] = [
      0, 1, 1, 1, 1, 1, 0, 0, 1, 1, 4, 4, 4, 1, 1, 1, 1, 4, 4, 3, 4, 4, 1, 4, 1,
      4, 3, 4, 4, 4, 4, 4, 1, 4, 4, 4, 4, 4, 4, 4, 1, 1, 4, 4, 4, 4, 4, 4, 1, 4,
      1, 4, 4, 4, 4, 4, 1, 1, 1, 1, 4, 4, 4, 4,
    ].map((value, i) => {
  ...
  generateRandomLevelData(levelIndex: number): LevelData | undefined {
    ...
    if (levelIndex == 0) {
      ...
      if (isFirstEverPlaySession) {
        console.log(
          'Set gamemode to "tutorial" so that the first playthrough is easier',
        );
    ...
    console.log(
      'map gen: caveParams (to learn why some levels are too small)',
      caveParams,
    );
    const { map, limits } = generateCave(
      caveParams || caveSizes.small,
      biome,
      this,
    );
    ...
    let validSpawnCoords: Vec2[] = tiles.flatMap((t) =>
      t && t.image == finalTileImages.all_ground ? [t] : [],
    );
    ...
    levelData.imageOnlyTiles = tiles.flatMap((x) =>
      x == undefined ? [] : [x],
    );
    ...
    for (let i = 0; i < numberOfPickups; i++) {
      ...
      if (choice) {
        const { index } = choice;
        const validSpawnCoordsIndex = randInt(
          0,
          validSpawnCoords.length - 1,
          this.random,
        );
    ...
    if (levelIndex >= 5) {
      const bluePortalPickupIndex = Pickup.pickups.findIndex(
        (p) => p.name == Pickup.BLUE_PORTAL,
      );
      if (bluePortalPickupIndex) {
        if (validSpawnCoords.length >= 2) {
          const numberOfBluePortals =
            levelIndex < 8 ? 2 : 2 + Math.floor((levelIndex - 8) / 2) * 2;
          for (
            let bluePortal = 0;
            bluePortal < numberOfBluePortals;
            bluePortal++
          ) {
            const validSpawnCoordsIndex = randInt(
              0,
    ...
    if (levelIndex > 2) {
      ...
      if (doSpawnUrns) {
        ...
        const typeOfUrn = chooseOneOfSeeded(
          [urn_explosive_id, urn_ice_id, urn_poison_id],
          this.random,
        );
        if (typeOfUrn) {
    ...
    const numberOfMinibossesAllowed = Math.ceil(
      Math.max(0, (levelIndex - 4) / 4),
    );
    ...
    for (let id of unitIds) {
      ...
      const coord = validSpawnCoords.splice(validSpawnCoordsIndex, 1)[0];
      const sourceUnit = allUnits[id];
      const { unitMinLevelIndexSubtractor } =
        unavailableUntilLevelIndexDifficultyModifier(this);
      ...
      if (coord) {
        const isMiniboss = !minibossAllowed
          ? false
          : numberOfMinibossesAllowed > numberOfMinibossesMade;
        if (isMiniboss) {
  ...
  pickGroundTileLayers(biome: Biome): string[] {
    ...
    return [
      baseTileChoice ? baseTileChoice.path : baseTile,
      layer2Choice ? layer2Choice.path : '',
      layer3Choice ? layer3Choice.path : '',
    ];
  }
  addGroundTileImages(biome: Biome) {
    ...
    for (let tile of this.imageOnlyTiles.filter(
      (t) => t.image.indexOf('liquid') === -1,
    )) {
      if (tile.image) {
    ...
    if (!this.lastLevelCreated) {
      console.error(
        'cannot lay down wall tiles, no lastLevelCreated to get biome from',
      );
      return;
    }
    ...
    for (let tile of this.imageOnlyTiles.filter(
      (t) =>
        !t.image.includes(`tiles/${this.lastLevelCreated?.biome}/all_ground`),
    )) {
      if (tile.image) {
        if (
          tile.image == `tiles/${this.lastLevelCreated.biome}/all_liquid.png`
        ) {
        ...
        const sprite = addPixiSprite(
          tile.image,
          isWall ? containerUnits : containerBoard,
        );
        if (sprite) {
          sprite.x = tile.x - config.COLLISION_MESH_RADIUS;
          sprite.y = tile.y - config.COLLISION_MESH_RADIUS;
  ...
  isPointValidSpawn(
    ...
    if (fromSource) {
      ...
      if (
        [...this.walls, ...this.liquidBounds].some((wall) =>
          lineSegmentIntersection({ p1: fromSource, p2: spawnPoint }, wall),
        )
      ) {
        return false;
      }
    ...
    if (
      [...this.walls, ...this.liquidBounds].some((wall) => {
        const rightAngleIntersection =
          findWherePointIntersectLineSegmentAtRightAngle(spawnPoint, wall);
        return (
          rightAngleIntersection &&
          math.distance(rightAngleIntersection, spawnPoint) <= radius
        );
      })
    ) {
      return false;
    ...
    if (
      entities.some((entity) =>
        Vec.equal(Vec.round(entity), Vec.round(spawnPoint)),
      )
    ) {
      return false;
    }
  ...
  findValidSpawn(
    spawnSource: Vec2,
    ringLimit: number,
    radius: number = config.COLLISION_MESH_RADIUS,
  ): Vec2 | undefined {
    const honeycombRings = ringLimit;
    for (let s of math.honeycombGenerator(
      radius,
      spawnSource,
      honeycombRings,
    )) {
  ...
  findValidSpawns(
    spawnSource: Vec2,
    radius: number = config.COLLISION_MESH_RADIUS / 4,
    ringLimit: number,
  ): Vec2[] {
  ...
  createLevelSyncronous(levelData: LevelData) {
    ...
    this.cacheWalls(
      obstacles,
      imageOnlyTiles.filter((x) => x.image == ''),
    );
        ...
        Pickup.create(
          {
            pos: p.coord,
            pickupSource: pickup,
            logSource: 'createLevelSyncronous',
          },
          this,
          false,
        );
    ...
    queueCenteredFloatingText(['Level', this.getLevelText()], 'white');
  ...
  async generateLevelData(levelIndex: number): Promise<void> {
    ...
    if (this.generatingLevel) {
      console.warn(
        'Setup: Shortcircuit generateLevelData; another level is already in the process of being generated',
      );
      return;
    }
    ...
    return new Promise<LevelData>((resolve) => {
      ...
      setTimeout(() => {
        if (this.lastLevelCreated?.levelIndex == levelIndex) {
          ...
          console.warn(
            'Setup: Shortcircuit generateLevelData; returning already generated level data',
          );
        } else {
  ...
  checkPickupCollisions(unit: Unit.IUnit, prediction: boolean) {
    for (let pu of prediction && this.pickupsPrediction
      ? this.pickupsPrediction
      : this.pickups) {
  ...
  isCoordOnWallTile(coord: Vec2): boolean {
    ...
    return (
      !!originalTile &&
      (originalTile.image === undefined ||
        originalTile.image == '' ||
        originalTile.image.includes('wall'))
    );
  }
  getMousePos(): Vec2 {
  ...
  isGameOver(): boolean {
    ...
    const isAllyNPCAlive =
      !useKillSwitch &&
      this.units
        .filter(
          (u) =>
            u.unitType == UnitType.AI &&
            playerFactions.includes(u.faction) &&
            u.unitSubType !== UnitSubType.DOODAD,
        )
        .some((u) => u.alive);
    // Note: unspawned players still own an "alive" unit
    const isConnectedPlayerAlive = this.players
      .filter((p) => p.clientConnected && p.unit.alive)
      .some((p) => p.unit.alive);
    if (useKillSwitch) {
      console.error(
        'WARN: Used Ally NPC win attempt kill switch to cause game over.  The ally npcs did not make any progress after a set number of turns.  This prevents an infinite loop.',
      );
    }
    return (
      this.players.length !== 0 && !isConnectedPlayerAlive && !isAllyNPCAlive
    );
  }
  updateGameOverModal() {
    // Add stats to modal:
...
${CardUI.cardListToImages(player.stats.longestSpell)}
      ...
      if (isOver) {
        ...
        if (!this.isRestarting) {
          ...
          console.log(
            '-------------------Host app game over',
            isOver,
            `restarting in ${Math.floor(millisTillRestart / 1000)} seconds`,
          );
          this.isRestarting = setTimeout(() => {
            const newUnderworld = new Underworld(
              overworld,
              pie,
            ...
            newUnderworld.lastLevelCreated =
              newUnderworld.generateLevelDataSyncronous(0, this.gameMode);
  ...
  tryEndPlayerTurnPhase(): boolean {
    ...
    if (
      ...
      if (activePlayers.every((p) => p.endedTurn)) {
        doEndPlayerTurnPhase = true;
      } else {
        console.log(
          'PlayerTurn: Check end player turn phase; players havent ended turn yet:',
          activePlayers.filter((p) => !p.endedTurn).map((p) => p.clientId),
        );
      }
    }
    // If all connected players are dead
    if (
      this.players.filter((p) => p.clientConnected).every((p) => !p.unit.alive)
    ) {
    ...
    if (doEndPlayerTurnPhase) {
        ...
        if (this.lastLevelCreated) {
          ...
          if (!u.flaggedForRemoval) {
            ...
            if (!originalTile || originalTile.image == '') {
              console.error(
                'Unit was force killed because they ended up out of bounds',
                u.unitSubType,
              );
      ...
      if (
        this.units.filter(
          (u) =>
            u.alive && u.faction == Faction.ALLY && u.unitType == UnitType.AI,
        ).length !== 0
      ) {
  ...
  async endFullTurnCycle() {
    ...
    for (let unit of this.units.filter((u) => u.unitType === UnitType.AI)) {
      await Promise.all(
        unit.onTurnEndEvents.map(async (eventName) => {
    ...
    for (let unit of this.units.filter(
    ...
    for (let player of this.players) {
      ...
      if (!Player.ableToAct(player)) {
        ...
        if (numberOfHotseatPlayers > 1 && player !== globalThis.player) {
          console.log(
            'Hotseat: Skip ending turn for a hotseat player because it is not currently their turn',
          );
          continue;
        }
        // Skip them
      ...
      if (player == globalThis.player && globalThis.player.isSpawned) {
        ...
        if (
          !globalThis.isHUDHidden &&
          !document.body?.classList.contains('hide-card-holders')
        ) {
          playSFXKey('yourTurn');
        }
      }
      ...
      const onTurnStartEventResults: boolean[] = await Promise.all(
        player.unit.onTurnStartEvents.map(async (eventName) => {
      ...
      if (onTurnStartEventResults.some((b) => b)) {
        ...
        console.log(
          `Force end player turn, player ${player.clientId} due to onTurnStartEvent`,
        );
        this.endPlayerTurn(player.clientId);
      ...
      if (!player.unit.alive) {
        console.log(
          `Force end player turn, player ${player.clientId} died when their turn started`,
        );
        this.endPlayerTurn(player.clientId);
        // Do not continue with initialization for this player
        continue;
  ...
  async endMyTurnButtonHandler() {
    if (globalThis.player) {
      ...
      if (elEndTurnBtn.classList.contains('upgrade')) {
        const upgradesLeftToChoose = this.upgradesLeftToChoose(
          globalThis.player,
        );
        if (upgradesLeftToChoose > 0) {
          ...
          console.error(
            'Unexpected: Cannot choose upgrades, upgrades left to choose:',
            upgradesLeftToChoose,
          );
        }
      ...
      if (this.isMyTurn()) {
        ...
        if (
          ...
          if (
            ...
            affirm = await Jprompt({
              text: 'Are you sure you want to end your turn without moving or casting?',
              noBtnText: 'Cancel',
              noBtnKey: 'Escape',
              yesText: 'End Turn',
              yesKey: 'Space',
  ...
  async endPlayerTurn(clientId: string) {
    ...
    if (!player) {
      console.error(
        'Cannot end turn, player with clientId:',
        clientId,
        'does not exist',
      );
      return;
    ...
    if (this.turn_phase != turn_phase.PlayerTurns) {
      ...
      console.info(
        "Cannot end the turn of a player when it isn't currently their turn",
      );
      return;
    }
    ...
    if (this.turn_phase === turn_phase.PlayerTurns) {
      ...
      if (!player.endedTurn) {
        ...
        // Trigger onTurnEnd Events
        await Promise.all(
          player.unit.onTurnEndEvents.map(async (eventName) => {
            const fn = Events.onTurnEndSource[eventName];
      ...
      const allEnemiesAreDead = this.units
        .filter(
          (u) =>
            u.faction == Faction.ENEMY && u.unitSubType !== UnitSubType.DOODAD,
        )
        .every((u) => !u.alive);
      const deadNonPortaledPlayers = this.players.filter(
        (p) => !Player.inPortal(p) && !p.unit.alive,
      );
      ...
      if (numberOfHotseatPlayers > 1) {
        ...
        setTimeout(async () => {
          ...
          if (this.players[0]) {
            ...
            console.error(
              'Hotseat: Tried to change player but player is undefined',
            );
          }
          CardUI.recalcPositionForCards(globalThis.player, this);
          ...
          if (
            globalThis.player &&
            this.upgradesLeftToChoose(globalThis.player)
          ) {
      ...
      console.error(
        'turn_phase must be PlayerTurns to end turn.  Cannot be ',
        this.turn_phase,
      );
    }
    Player.syncLobby(this);
  ...
  chooseUpgrade(player: Player.IPlayer, upgrade: Upgrade.IUpgrade) {
    ...
    if (player == globalThis.player) {
      document.body
        ?.querySelector(`.card[data-upgrade="${upgrade.title}"]`)
        ?.classList.toggle('chosen', true);
  ...
  upgradesLeftToChoose(player: Player.IPlayer): number {
    ...
    return (
      this.cardDropsDropped +
      config.STARTING_CARD_COUNT -
      player.inventory.filter(
        (spellId) => (globalThis.freeSpells || []).indexOf(spellId) == -1,
      ).length
    );
  }
  spendStatPoint(stat: string, player: Player.IPlayer) {
    ...
    if (stat == 'Good Looks') {
      ...
      this.units
        .filter((u) => u.unitType == UnitType.AI)
        .forEach((u) =>
          Unit.takeDamage(
            u,
            u.healthMax * damageMultiplier,
            undefined,
            this,
            false,
          ),
      ...
      switch (player.mageType) {
        ...
        case 'Far Gazer':
          ...
          statBumpAmount.staminaMax = Math.floor(
            (statBumpAmount.staminaMax as number) / 2,
          );
          break;
      }
    ...
    if (player) {
      ...
      if (upgrades.length) {
        const elUpgrades = upgrades.map((upgrade) =>
          Upgrade.createUpgradeElement(upgrade, player, this),
        );
  ...
  showUpgrades() {
    ...
    if (document.body?.classList.contains(showUpgradesClassName)) {
      console.log(
        'showUpgrades: showUpgrades was called but it is already visible so this function returns immediately to avoid regenerating upgrades',
      );
      return;
    }
    if (!player) {
      ...
      if (!globalThis.headless) {
        console.error(
          'showUpgrades: Cannot show upgrades, no globalThis.player',
        );
      }
      return;
    }
    ...
    if (
      upgradesLeftToChoose <= 0 &&
      perksLeftToChoose <= 0 &&
      cursesLeftToChoose <= 0
    ) {
    ...
    if (
      upgradesLeftToChoose > 0 &&
      player.inventory.length < config.STARTING_CARD_COUNT
    ) {
    ...
    if (elUpgradePickerLabel) {
      const pickingClass = globalThis.player
        ? Upgrade.isPickingClass(globalThis.player)
        : false;
    ...
    if (elUpgradePickerLabel) {
      const hotseatPlayerName =
        globalThis.numberOfHotseatPlayers > 1 ? `${player.name}: ` : '';
      elUpgradePickerLabel.innerHTML =
        hotseatPlayerName + elUpgradePickerLabel?.innerHTML;
    }
    ...
    if (!elUpgradePicker || !elUpgradePickerContent) {
      console.error(
        'showUpgrades: elUpgradePicker or elUpgradePickerContent are undefined.',
      );
    }
    if (player) {
      ...
      if (isPerk) {
        if (isCursePerk) {
          const mostUsedLastLevelCards = Object.entries(player.spellState || {})
            .sort((a, b) => b[1].count - a[1].count)
            // Remove cards that are already affected by calamity
            .filter(
              ([_spellId, spellState]) =>
                !(spellState.disabledUntilLevel > this.levelIndex),
            )
            .slice(0, 3);
          if (mostUsedLastLevelCards.length == 0) {
            ...
            const elPerks = mostUsedLastLevelCards
              .slice(0, 2)
              .map((count) =>
                createCursePerkElement({ cardId: count[0] }, this),
              );
            for (let i = 0; i < 2; i++) {
          ...
          if (elUpgradePickerContent) {
            ...
            const statValueModifier = (
              ...
              if (stat == 'attackRange') {
                ...
                return Math.floor(
                  (100 * value) / config.PLAYER_BASE_ATTACK_RANGE,
                );
              }
              return value;
            };
...
<div class="card upgrade perk ui-border pick-stats">
  ...
  ${['healthMax', 'manaMax', 'staminaMax', 'attackRange', 'Good Looks']
    ...
    .join('')}
            ...
            elUpgradePickerContent
              .querySelectorAll('.stat-row .plus-btn-container')
              .forEach((el) => {
        ...
        const upgrades = Upgrade.generateUpgrades(
          player,
          numberOfUpgradesToChooseFrom,
          minimumProbability,
          this,
        );
        if (!upgrades.length) {
          ...
          if (elUpgradePickerContent) {
            ...
            for (let elUpgrade of elUpgrades) {
              if (elUpgrade) {
                elUpgradePickerContent.appendChild(elUpgrade);
                if (
                  globalThis.devAutoPickUpgrades &&
                  elUpgrade == elUpgrades[0]
                ) {
                  elUpgrade.click();
                }
              } else {
                console.warn(
                  'showUpgrades: Upgrade is undefined, this block should never be executed in headless mode',
                );
              }
            }
          }
          ...
          if (
            numberOfUpgradesToChooseFrom > 1 &&
            (!upgrades[0] || upgrades[0].type !== 'mageType')
          ) {
      ...
      console.error(
        'showUpgrades: Upgrades cannot be generated, player not found',
      );
    }
  }
  ...
  checkForEndOfLevel(): boolean {
    ...
    const areAllLivingPlayersInPortal =
      livingSpawnedPlayers.filter(Player.inPortal).length ===
      livingSpawnedPlayers.length;
    ...
    // can't get out of
    const hotseatOverride =
      numberOfHotseatPlayers > 1 && livingSpawnedPlayers.some(Player.inPortal);
    // Advance the level if there are living players and they all are in the portal:
    if (
      hotseatOverride ||
      (livingSpawnedPlayers.length && areAllLivingPlayersInPortal)
    ) {
      console.log(
        'All living, spawned players are inPortal, go to the next level',
      );
      ...
      setTimeout(() => {
        ...
        if (globalThis.isHost(this.pie)) {
          ...
          console.log(
            'This instance is not host, host will trigger next level generation.',
          );
        }
      }, 0);
  ...
  tryRestartTurnPhaseLoop() {
    ...
    if (
      ...
      this.players.some((player) => Player.ableToAct(player))
    ) {
      ...
      switch (phase) {
        case turn_phase[turn_phase.PlayerTurns]:
          if (this.players.every((p) => !p.clientConnected)) {
            ...
            for (let u of this.units.filter(
              (u) => u.unitType == UnitType.PLAYER_CONTROLLED,
            )) {
        ...
        case turn_phase[turn_phase.NPC_ALLY]:
          ...
          for (let unit of this.units.filter(
            (u) => u.unitType == UnitType.AI && u.faction == Faction.ALLY,
          )) {
        ...
        case turn_phase[turn_phase.NPC_ENEMY]:
          ...
          if (
            ...
            console.log(
              'Turn Management: Skipping executingNPCTurn for Faction.ENEMY',
            );
          }
          await this.endFullTurnCycle();
          // At the end of their turn, deal damage if still in liquid
          for (let unit of this.units.filter(
            (u) => u.unitType == UnitType.AI && u.faction == Faction.ENEMY,
          )) {
            if (unit.inLiquid && unit.alive) {
              doLiquidEffect(this, unit, false);
              floatingText({
  ...
  incrementTargetsNextTurnDamage(
    targets: Unit.IUnit[],
    damage: number,
    canAttack: boolean,
  ) {
  ...
  redPortalBehavior(faction: Faction) {
    const portalName =
      faction == Faction.ENEMY ? Pickup.RED_PORTAL : Pickup.BLUE_PORTAL;
    const deathmasons = this.units.filter(
      (u) => u.unitSourceId == bossmasonUnitId && u.faction == faction,
    );
    for (let deathmason of deathmasons) {
      const seed = seedrandom(deathmason.id.toString());
      const deathmasonPortals = this.pickups.filter(
        (p) => p.name == portalName && !p.flaggedForRemoval,
      );
      const deathmasonPortalTeleportIndex = randInt(
        0,
        deathmasonPortals.length,
        seed,
      );
  ...
  async executeNPCTurn(faction: Faction) {
    ...
    const cachedTargets: {
      [id: number]: { targets: Unit.IUnit[]; canAttack: boolean };
    } = {};
    ...
    for (let subTypes of [
      [
        UnitSubType.RANGED_LOS,
        UnitSubType.RANGED_RADIUS,
        UnitSubType.SUPPORT_CLASS,
      ],
      [UnitSubType.MELEE],
      [UnitSubType.SPECIAL_LOS],
      [UnitSubType.DOODAD],
    ]) {
      ...
      unitloop: for (let u of this.units.filter(
        (u) =>
          ...
          u.alive &&
          u.faction == faction &&
          subTypes.includes(u.unitSubType),
      )) {
        ...
        const unitSource = allUnits[u.unitSourceId];
        if (unitSource) {
          const { targets, canAttack } = cachedTargets[u.id] || {
            targets: [],
            canAttack: false,
          };
          // Add unit action to the array of promises to wait for
          let promise = raceTimeout(
            5000,
            `Unit.action; unitSourceId: ${u.unitSourceId}; subType: ${u.unitSubType}`,
            unitSource
              .action(u, targets, this, canAttack)
              .then(async (actionResult) => {
                ...
                if (u.unitSubType !== UnitSubType.MELEE && u.inLiquid) {
                  const seed = seedrandom(
                    `${this.seed}-${this.turn_number}-${u.id}`,
                  );
  ...
  canUnitAttackTarget(u: Unit.IUnit, attackTarget?: Unit.IUnit): boolean {
    ...
    switch (u.unitSubType) {
      case UnitSubType.MELEE:
        ...
        if (math.distance(u, attackTarget) <= maxPathDistance) {
          ...
          if (u.path && u.path.points.length) {
            ...
            if (
              lastPointInPath &&
              !withinMeleeRange({ ...u, ...lastPointInPath }, attackTarget)
            ) {
      ...
      case UnitSubType.SPECIAL_LOS:
        return (
          u.alive &&
          this.hasLineOfSight(u, attackTarget) &&
          Unit.inRange(u, attackTarget)
        );
      ...
      case UnitSubType.RANGED_RADIUS:
        return (
          u.alive && Unit.inRange(u, attackTarget) && u.mana > u.manaCostToCast
        );
      ...
      default:
        console.error(
          'Cannot determine canUnitAttackTarget, unit sub type is unaccounted for',
          u.unitSubType,
        );
        return false;
    }
  }
  getEntitiesWithinDistanceOfTarget(
    target: Vec2,
    distance: number,
    prediction: boolean,
  ) {
  ...
  getPickupsWithinDistanceOfTarget(
    ...
    const pickups =
      prediction && this.pickupsPrediction
        ? this.pickupsPrediction
        : this.pickups;
  ...
  getUnitsAt(coords: Vec2, prediction?: boolean): Unit.IUnit[] {
    const sortedByProximityToCoords = (
      prediction && this.unitsPrediction ? this.unitsPrediction : this.units
    )
  ...
  getPickupAt(coords: Vec2, prediction?: boolean): Pickup.IPickup | undefined {
    const sortedByProximityToCoords = (
      ...
      .filter(
        (p) =>
          !isNaN(p.x) && !isNaN(p.y) && math.distance(coords, p) <= p.radius,
      )
      .sort((a, b) => math.distance(a, coords) - math.distance(b, coords));
  ...
  getDoodadAt(coords: Vec2, prediction?: boolean): Doodad.IDoodad | undefined {
    const sortedByProximityToCoords = (
      prediction && this.doodadsPrediction
        ? this.doodadsPrediction
        : this.doodads
    )
  ...
  async castCards(args: CastCardsArgs): Promise<Cards.EffectState> {
    const {
      casterCardUsage,
      casterUnit,
    ...
    if (
      !prediction &&
      casterUnit == (globalThis.player && globalThis.player.unit)
    ) {
    ...
    if (!prediction) {
      ...
      tutorialCompleteTask(
        ...
        () => casterUnit.mana < casterUnit.manaMax,
      );
    }
    ...
    if (useInitialTarget) {
      ...
      if (effectState.initialTargetedUnitId !== undefined) {
        const initialTargetUnit = this.units.find(
          (u) => u.id === effectState.initialTargetedUnitId,
        );
        if (initialTargetUnit) {
          Cards.addTarget(initialTargetUnit, effectState);
        } else {
          console.error(
            'effectState.initialTargetedUnitId was defined but the unit was not found',
          );
        }
      } else {
      ...
      if (effectState.initialTargetedPickupId !== undefined) {
        const initialTargetPickup = this.pickups.find(
          (p) => p.id === effectState.initialTargetedPickupId,
        );
        if (initialTargetPickup) {
          Cards.addTarget(initialTargetPickup, effectState);
        } else {
          console.error(
            'effectState.initialTargetedPickupId was defined but the unit was not found',
          );
        }
      } else {
    ...
    const castingParticleEmitter = makeRisingParticles(
      effectState.casterUnit,
      prediction,
      hexToString(magicColor || 0xffffff),
      -1,
    );
    ...
    for (let index = 0; index < effectState.cardIds.length; index++) {
      ...
      if (card) {
        ...
        // is hard-coded here
        excludedTargets = excludedTargets.concat(
          effectState.targetedUnits.filter((u) => {
        ...
        effectState.targetedUnits = effectState.targetedUnits.filter(
          (u) => !excludedTargets.includes(u),
        );

        const cardEffectPromise = card.effect(
          effectState,
          card,
          quantity,
          this,
        ...
        // Await the cast
        try {
          effectState = await reportIfTakingTooLong(
            10000,
            `${card.id};${prediction}`,
            cardEffectPromise,
          );
        ...
        if (!effectState.shouldRefundLastSpell) {
          ...
          if (!prediction && effectState.casterPlayer && card.cooldown) {
            Object.assign(effectState.casterPlayer.spellState[card.id] || {}, {
          ...
          for (let i = 0; i < quantity; i++) {
            const timesUsedSoFar =
              (casterCardUsage[card.id] || 0) +
              (quantity > 1 ? i * card.expenseScaling : i);
            const singleCardCost = calculateCostForSingleCard(
              card,
              timesUsedSoFar,
              casterPlayer,
            );
          ...
          // have to do for now
          if (!prediction) {
            effectState.casterUnit.mana = Math.max(
              0,
              effectState.casterUnit.mana,
            );
          }
  ...
  checkIfShouldSpawnPortal() {
    ...
    if (
      this.units
        ...
        .every((u) => !u.alive) &&
      this.players.some((p) => p.isSpawned && p.clientConnected)
    ) {
      ...
      if (loopIndex > this.wave) {
        ...
        this.units
          .filter((u) => u.unitType == UnitType.AI && !u.alive)
          .forEach((u) => {
            Unit.addModifier(u, corpseDecayId, this, false);
          });
        const unitIds = getEnemiesForAltitude2(this, this.levelIndex);
        const numberOfMinibossesAllowed = Math.ceil(
          Math.max(0, (this.levelIndex - 4) / 4),
        );
        let numberOfMinibossesMade = 0;
        // Trick for finding valid spawnable tiles
        const validSpawnCoords =
          this.lastLevelCreated?.imageOnlyTiles.filter((x) =>
            x.image.endsWith('all_ground.png'),
          ) || [];
      ...
      this.pickups
        .filter(
          (p) =>
            p.name !== Pickup.CARDS_PICKUP_NAME &&
            p.name !== Pickup.PICKUP_PORTAL_NAME,
        )
        .forEach((p) => {
          makeScrollDissapearParticles(p, false);
          Pickup.removePickup(p, this, false);
      ...
      const portalPickup = Pickup.pickups.find(
        (p) => p.name == Pickup.PICKUP_PORTAL_NAME,
      );
      if (portalPickup) {
        const portalsAlreadySpawned = !!this.pickups
          .filter((p) => !p.flaggedForRemoval && !isNaN(p.x) && !isNaN(p.x))
          .find((p) => p.name === Pickup.PICKUP_PORTAL_NAME);
        if (!portalsAlreadySpawned) {
          for (let playerUnit of this.units.filter(
            (u) => u.unitType == UnitType.PLAYER_CONTROLLED && u.alive,
          )) {
            const portalSpawnLocation =
              this.findValidSpawn(playerUnit, 4) || playerUnit;
            if (!isOutOfBounds(portalSpawnLocation, this)) {
              Pickup.create(
                {
                  pos: portalSpawnLocation,
                  pickupSource: portalPickup,
                  logSource: 'Portal',
                },
                this,
                false,
          ...
          console.log(
            'Portals have already been spawned, do not spawn additional',
          );
        }
      } else {
  ...
  dev_shuffleUnits() {
    ...
    while (currentIndex != 0) {
      ...
      [this.units[currentIndex], this.units[randomIndex]] = [
        this.units[randomIndex],
        this.units[currentIndex],
      ];
    }
  ...
  unitIsIdentical(unit: Unit.IUnit, serialized: Unit.IUnitSerialized): boolean {
    return (
      unit.id == serialized.id && unit.unitSourceId == serialized.unitSourceId
    );
  }
  syncUnits(
    units: Unit.IUnitSerialized[],
    excludePlayerUnits: boolean = false,
  ) {
    ...
    console.log(
      ...
      units.map((u) => u.id),
      'current units:',
      this.units.map((u) => u.id),
    );
    // Report detected duplicate id:
    this.units.forEach((unit, index) => {
      if (this.units.findIndex((u) => u.id == unit.id) !== index) {
        console.log(
          'Duplicate unit id in: ',
          this.units.map((u) => `${u.id}:${u.unitSourceId}`),
        );
        console.log(
          'Syncing to: ',
          units.map((u) => `${u.id}:${u.unitSourceId}`),
        );
    ...
    for (let i = 0; i < units.length; i++) {
      ...
      if (serializedUnit) {
        if (
          excludePlayerUnits &&
          serializedUnit.unitType == UnitType.PLAYER_CONTROLLED
        ) {
          continue;
        }
        if (currentUnit) {
          ...
          if (
            this.unitIsIdentical(currentUnit, serializedUnit) &&
            currentUnit.image?.sprite.parent !== null
          ) {
  ...
  syncPlayers(players: Player.IPlayerSerialized[]) {
    console.log(
      ...
      JSON.stringify(players.map((p) => p.clientId)),
    );
  ...
  pickupIsIdentical(
    ...
    serialized: Pickup.IPickupSerialized,
  ): boolean {
  ...
  syncPickups(pickups: Pickup.IPickupSerialized[]) {
    ...
    console.log(
      'sync: Syncing pickups',
      pickups.map((u) => `${u.id}:${u.flaggedForRemoval}`),
      'current pickups:',
      this.pickups.map((u) => `${u.id}:${u.flaggedForRemoval}`),
    );
    ...
    for (let i = 0; i < pickups.length; i++) {
      ...
      if (serializedPickup) {
        const currentPickup = this.pickups.find(
          (p) => p.id == serializedPickup.id,
        );
        if (currentPickup) {
          ...
          if (
            this.pickupIsIdentical(currentPickup, serializedPickup) &&
            currentPickup.image?.sprite.parent !== null
          ) {
            const {
              x,
              y,
  ...
  async mergeExcessUnits() {
    ...
    for (let faction of [Faction.ALLY, Faction.ENEMY]) {
      ...
      if (factionedUnitsOverLimit > 0) {
        const mergesToDo =
          factionedUnitsOverLimit / config.NUMBER_OF_UNITS_TO_MERGE;
        ...
        const unitsGroupedByUnitSourceId = factionedUnits.reduce<{
          [unitSourceId: string]: Unit.IUnit[];
        }>((groups, unit) => {
        ...
        for (let [unitSourceId, units] of Object.entries(
          unitsGroupedByUnitSourceId,
        )) {
          ...
          if (mergeMapKeys.includes(unitSourceId)) {
            ...
            for (let u of units) {
              if (!currentCandidate) {
                ...
                if (
                  currentCandidate.mergeUnits.length <
                  config.NUMBER_OF_UNITS_TO_MERGE
                ) {
  ...
  async merge(unit: Unit.IUnit, mergeUnits: Unit.IUnit[]): Promise<boolean> {
    ...
    if (unitSourceId) {
      ...
      floatingText({
        coords: unit,
        text: 'This unit is not setup to be merged',
      });
      return false;
    }
...
type IUnderworldSerialized = Omit<
  typeof Underworld,
  | 'pie'
  | 'overworld'
  | 'prototype'
...
type UnderworldNonFunctionProperties = Exclude<
  NonFunctionPropertyNames<Underworld>,
  null | undefined
>;
export type IUnderworldSerializedForSyncronize = Omit<
  Pick<Underworld, UnderworldNonFunctionProperties>,
  | 'pie'
  | 'overworld'
  | 'debugGraphics'
  | 'players'
...
function getEnemiesForAltitude2(
  ...
  const {
    unitMinLevelIndexSubtractor,
    budgetMultiplier: difficultyBudgetMultiplier,
  } = unavailableUntilLevelIndexDifficultyModifier(underworld);
  let possibleUnitsToChoose = Object.values(allUnits)
    ...
    .map((u) => ({
      id: u.id,
      probability: u.spawnParams?.probability || 1,
      budgetCost: u.spawnParams?.budgetCost || 1,
    }));
  const unitTypes = Array(numberOfTypesOfEnemies)
    .fill(null)
    // flatMap is used to remove any undefineds
    .flatMap(() => {
      const chosenUnitType = chooseObjectWithProbability(
        possibleUnitsToChoose,
        underworld.random,
      );
      // Remove chosen Unit type from pick source
      if (chosenUnitType) {
        possibleUnitsToChoose = possibleUnitsToChoose.filter(
          (u) => u.id !== chosenUnitType.id,
        );
  ...
  if (
    connectedClients.length > config.NUMBER_OF_PLAYERS_BEFORE_BUDGET_INCREASES
  ) {
    ...
    console.log(
      ...
      budgetMultiplier,
      ' due to the number of players connected',
    );
  ...
  while (budgetLeft > 0) {
    ...
    for (let chosenUnitType of unitTypes
      ...
      const maxNumberOfThisUnit = Math.min(
        Math.max(levelIndex, 1),
        Math.floor((totalBudget * 0.7) / chosenUnitType.budgetCost),
      );
...
function getEnemiesForAltitude(
  underworld: Underworld,
  levelIndex: number,
): string[] {
  ...
  const possibleUnitsToChoose = Object.values(allUnits)
    .filter(
      (u) =>
        u.spawnParams && u.spawnParams.unavailableUntilLevelIndex <= levelIndex,
    )
    .map((u) => ({
      id: u.id,
      probability: u.spawnParams ? u.spawnParams.probability : 0,
    }));
  const unitIds = Array(numberOfUnits)
    .fill(null)
    // flatMap is used to remove any undefineds
    .flatMap(() => {
      const chosenUnit = chooseObjectWithProbability(
        possibleUnitsToChoose,
        underworld.random,
      );

...
import {
  closestLineSegmentIntersectionWithLine,
...
export function findSafeFallInPoint(
  currentPosition: Vec2,
  nextPosition: Vec2,
  underworld: Underworld,
): { safeFallInPosition: Vec2; hitLava: boolean } {
  ...
  const intersectionInfo = closestLineSegmentIntersectionWithLine(
    { p1: currentPosition, p2: nextPosition },
    underworld.liquidBounds,
  );
  if (intersectionInfo) {
    const { intersection, lineSegment: liquidBoundary } = intersectionInfo;

    const dist = distance(nextPosition, intersection);
    let fallInPoint = similarTriangles(
      nextPosition.x - intersection.x,
      nextPosition.y - intersection.y,
      dist,
      fallInDistance,
    );
    ...
    if (isOutside && magnitude(diffFromP1) <= fallInThreshold) {
      ...
      const fromCorner = similarTriangles(
        deltaX,
        deltaY,
        magnitude(subtract(liquidBoundary.p1, liquidBoundary.p2)),
        fallInDistance,
      );
      const fallInThresholdProjectedFromWallP1toWallP2 = subtract(
        liquidBoundary.p1,
        fromCorner,
      );
      // Rotate vector 90 degrees https://stackoverflow.com/a/4780141/4418836
      safeFallInPosition = add(fallInThresholdProjectedFromWallP1toWallP2, {
        x: -fromCorner.y,
        y: fromCorner.x,
      });
      ...
      const fromCorner = similarTriangles(
        deltaX,
        deltaY,
        magnitude(subtract(liquidBoundary.p2, liquidBoundary.p1)),
        fallInDistance,
      );
      const fallInThresholdProjectedFromWallP2toWallP1 = subtract(
        liquidBoundary.p2,
        fromCorner,
      );
      safeFallInPosition = add(fallInThresholdProjectedFromWallP2toWallP1, {
        x: fromCorner.y,
        y: -fromCorner.x,
      });
    } else {
...
function fallInOrOutToSafeDistanceFromEdge(
  entity: HasSpace,
  poly: Polygon2,
  underworld: Underworld,
) {
  ...
  for (let ls of lineSegments) {
    ...
    if (intersection) {
      ...
      if (distance(intersection, entity) <= radius) {
        ...
        let newPos = entity.inLiquid
          ? getCoordsAtDistanceTowardsTarget(entity, intersection, radius, true)
          : getCoordsAtDistanceTowardsTarget(
              intersection,
              entity,
              radius,
        ...
        if (entity.inLiquid) {
          ...
          for (let wall of underworld.walls) {
            ...
            if (intersection) {
              newPos = math.getCoordsAtDistanceTowardsTarget(
                intersection,
                entity,
                entity.radius,
              );
...
export function isCoordInLiquid(
  coord: Vec2,
  underworld: Underworld,
): Polygon2 | undefined {
...
export function tryFallInOutOfLiquid(
  entity: HasSpace,
  underworld: Underworld,
  prediction: boolean,
) {

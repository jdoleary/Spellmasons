...
const unit: UnitSource = {
  ...
  action: async (
    ...
    const portalName =
      unit.faction == Faction.ENEMY ? RED_PORTAL : Pickup.BLUE_PORTAL;
    const deathmasonPortals = underworld.pickups.filter(
      (p) => p.name == portalName,
    );
    const deathmasonPortalPickupSource = pickups.find(
      (p) => p.name == portalName,
    );
    if (deathmasonPortalPickupSource) {
      if (deathmasonPortals.length == 0 && unit.mana >= portalCastCost) {
        ...
        const keyMoment = () => {
          ...
          for (let coord of portalCoords) {
            ...
            lastPromise = makeManaTrail(
              ...
              coord,
              underworld,
              unit.faction == Faction.ENEMY ? '#930e0e' : '#0e0e93',
              '#ff0000',
            ).then(() => {
              const portal = Pickup.create(
                {
                  pos: coord,
                  pickupSource: deathmasonPortalPickupSource,
                  logSource: 'deathmason',
                },
                underworld,
                false,
              );
        ...
        await Unit.playComboAnimation(unit, 'playerAttackMedium0', keyMoment, {
          animationSpeed: 0.2,
          loop: false,
        });
        ...
        if (
          ...
          if (closestUnit) {
            ...
            await Unit.playComboAnimation(
              unit,
              'playerAttackSmall',
              keyMoment,
              { animationSpeed: 0.2, loop: false },
            );
          }
        }
      }
  ...
  getUnitAttackTargets: (unit: Unit.IUnit, underworld: Underworld) => {
    ...
    if (unit.unitType == UnitType.AI) {
      ...
      return unit.mana >= portalCastCost && deathmasonPortals.length == 0
        ? [unit]
        : [];
    }
    return [];
...
export function summonUnitAtPickup(
  faction: Faction,
  ...
  const enemyIsClose = underworld.units
    ...
    .some((u) => math.distance(pickup, u) <= config.PLAYER_BASE_ATTACK_RANGE);

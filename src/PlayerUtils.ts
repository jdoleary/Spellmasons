...
function isAllowedToCastOutOfRange(cardIds: string[]): boolean {
  ...
  return (
    cardIds[0] == targetArrowCardId ||
    cardIds.every((id) => id.toLowerCase().includes('arrow'))
  );
}
...
export function getEndOfRange(caster: Player.IPlayer, target: Vec2): Vec2 {
  return math.getCoordsAtDistanceTowardsTarget(
    caster.unit,
    target,
    caster.unit.attackRange,
  );
}
...
export function setPlayerNameUI(player: Player.IPlayer) {
  ...
  if (name !== undefined) {
    ...
    if (globalThis.pixi && player.unit.image) {
      ...
      nameText.style = {
        fill: 'white',
        fontSize: config.NAME_TEXT_DEFAULT_SIZE,
        fontFamily: 'Forum',
...
export const sendPlayerThinkingThrottled = throttle(
  (thoughts: { target?: Vec2; cardIds: string[] }, underworld: Underworld) => {
    ...
    if (underworld.isMyTurn()) {
      ...
      if (hash !== underworld.lastThoughtsHash) {
        if (underworld.pie) {
          ...
          const willClearThoughts =
            cardIds.length == 0 && underworld.lastThoughtsHash !== '';

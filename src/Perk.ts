...
export function getPerkText(
  perk: AttributePerk,
  omitWhen: boolean = false,
): string {
  return `
${perk.certainty < 1.0 ? `🎲 ${Math.round(perk.certainty * 100)}% chance` : ``}
${perkAttributeToIcon(perk.attribute)} +${perkAmountToText(
    perk.amount,
    perk.attribute,
  )} ${i18n(perkAttributeToString(perk.attribute))}
...
export function generateRandomStatCalamity(
  underworld: Underworld,
  index: number,
): StatCalamity | undefined {
  // health, damage, attack range
  const seedString =
    getUniqueSeedString(underworld, globalThis.player) +
    `-${index}-${player?.cursesChosen || 0}`;
  const seed = seedrandom(seedString);
  const currentLevelUnitSourceIds = [
    ...new Set(
      underworld.units
        .filter((u) => u.unitSubType !== UnitSubType.DOODAD)
        .map((u) => u.unitSourceId)
        .filter((uid) => uid !== spellmasonUnitId),
    ),
  ];
  const unitId: string | undefined = chooseOneOfSeeded(
    currentLevelUnitSourceIds,
    seedrandom(`-${index}-${seedString}-unit`),
  );
  const growthFactor = underworld.levelIndex - LAST_LEVEL_INDEX;
  const unitSource = allUnits[unitId || ''];
  if (unitSource) {
    const stats: (keyof Unit.IUnit)[] = [
      'healthMax',
      'attackRange',
      'staminaMax',
    ];
...
// Calamities / Calamity
export function createCursePerkElement(
  { cardId, statCalamity }: { cardId?: string; statCalamity?: StatCalamity },
  underworld: Underworld,
) {
  ...
  if (cardId) {
    ...
    const numberOfLevelsToDisable =
      2 + Math.floor((underworld.levelIndex - (1 + LAST_LEVEL_INDEX)) / 2);
    descriptionText.innerHTML = i18n([
      'disable spell',
      content.id,
      numberOfLevelsToDisable.toString(),
    ]);
    ...
    descriptionText.innerHTML = `${statCalamity.unitId} ${i18n([
      wordMap[statCalamity.stat] || '',
    ])} + ${statCalamity.percent}%`;
    desc.appendChild(descriptionText);
    element.addEventListener('click', (e) => {
      globalThis.timeLastChoseUpgrade = Date.now();
      // Prevent click from "falling through" upgrade and propagating to vote for overworld level
...
export function createPerkElement(
  perk: AttributePerk,
  player: IPlayer,
  underworld: Underworld,
) {
...
export type UpgradableAttribute =
  | 'staminaMax'
  | 'stamina'
  | 'healthMax'
  | 'health'
...
export function generatePerks(
  ...
  for (let i = 0; i < number; i++) {
    ...
    const seed = seedrandom(
      getUniqueSeedString(underworld, globalThis.player) +
        `-${i}-${player?.reroll || 0}-${failedDueToDuplicateCount}-${
          player?.attributePerks.length || 0
        }`,
    );
    const choiceAttributeType =
      chooseObjectWithProbability(
        [
          { attr: 'maxStat', probability: 10 },
          { attr: 'stat', probability: 3 },
        ],
        seed,
      )?.attr || 'maxStat';
    if (choiceAttributeType == 'maxStat') {
      attribute =
        chooseOneOfSeeded(
          ['staminaMax', 'healthMax', 'manaMax', 'attackRange'],
          seed,
        ) || 'stamina';
      when =
        chooseOneOfSeeded<WhenUpgrade>(['immediately', 'everyLevel'], seed) ||
        'immediately';
      amount = 15;
      ...
      attribute =
        chooseOneOfSeeded(['stamina', 'health', 'mana'], seed) || 'stamina';
      ...
      when =
        chooseOneOfSeeded<WhenUpgrade>(['everyLevel', 'everyTurn'], seed) ||
        'everyLevel';
      if (when == 'everyLevel') {
        amount = 40;
        certainty = 1.0;
    ...
    // Prevent duplicates
    const duplicate = perks.find(
      (p) => JSON.stringify(p) == JSON.stringify(newPerk),
    );
    if (duplicate) {
      i--;
      ...
      if (failedDueToDuplicateCount > 100) {
        console.error(
          'Infinite loop protection, could not generate unique perk',
        );
        return perks;
      }
      continue;
  ...
  if (!globalThis.headless) {
    if (elPerkList && elPerksEveryLevel && elPerksEveryTurn) {
      if (player.attributePerks.length) {
        ...
        const everyLevel = player.attributePerks.filter(
          (p) => p.when == 'everyLevel',
        );
        const everyTurn = player.attributePerks.filter(
          (p) => p.when == 'everyTurn',
        );
...
export function tryTriggerPerk(
  perk: AttributePerk,
  player: IPlayer,
  when: WhenUpgrade,
  random: seedrandom.PRNG,
  underworld: Underworld,
  offsetNotifyByMs: number,
) {
  if (perk.when == when) {
    ...
    if (doTriggerPerk) {
      if (
        perk.attribute == 'manaMax' ||
        perk.attribute == 'healthMax' ||
        perk.attribute == 'staminaMax'
      ) {
      ...
      if (player === globalThis.player) {
        setTimeout(() => {
          floatingText({
            coords: player.unit,
            text: `+${Math.round(
              player.unit[perk.attribute] - oldAttributeAmount,
            )} ${i18n(perkAttributeToString(perk.attribute))}`,
          });
        }, offsetNotifyByMs);
      }

  ...
  if (otherPrompts && otherPrompts.length) {
    ...
    otherPrompts.forEach((el) => {
      if (
        ...
        (el as HTMLElement).dataset.text === textId
      ) {
        earlyExit = true;
        return;
      }
    ...
    if (earlyExit) {
      console.trace(
        'Early exit jprompt, another prompt already exists with the same text.',
      );
      return false;
    }
  }
...
<div class="prompt-inner ${!imageSrc ? 'thin' : ''}">
    <div class="prompt-content">
        ${
          imageSrc
            ? `<img src="${imageSrc}" onerror="this.style.display='none';"/>`
            : ''
        }
        <div class="text">
    ...
    <div class="button-holder">
        <button class="yes button-wrapper" ${
          yesKey ? `data-key="${yesKey}"` : ''
        }>
        ...
        ${
          noBtnText
            ? `<button class="no button-wrapper" data-key="${noBtnKey}"> 
            <div class="button-inner">
...
<div class="prompt-inner ${!imageSrc ? 'thin' : ''}">
    ...
    <form style="width:auto">
        ...
        <div class="button-holder">
            ${
              noBtnText
                ? `<button type="button" class="no button-wrapper" data-key="${noBtnKey}"> 
                <div class="button-inner">
                    ${globalThis.i18n(noBtnText)}
                    <div class="hotkey-badge-holder">
                        <kbd class="hotkey-badge">${noBtnKey}</kbd>
            ...
            <button type="submit" class="yes button-wrapper" ${
              yesKey ? `data-key="${yesKey}"` : ''
            }>
                <div class="button-inner">
                    ${globalThis.i18n(yesText)}

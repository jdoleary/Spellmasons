    ...
    if (explainData) {
      ...
      if (forceShow || !explainData.condition || explainData.condition()) {
        ...
        setTimeout(() => {
          Jprompt({
            ...explainData.prompt(),
            portal: forceShow ? portal || undefined : undefined,
            forceShow,
          });
        }, 500);
...
const explainMap: { [key: string]: ExplainData } = {
  [EXPLAIN_WALK]: {
    prompt: () => ({
      imageSrc: 'images/explain/walk.gif',
      text: 'explain move',
      yesText: 'Okay',
    }),
  },
  [EXPLAIN_CAST]: {
    prompt: () => ({
      imageSrc: 'images/explain/cast.gif',
      text: ['explain cast', keyToHumanReadable(['Escape'])],
      yesText: 'Nice!',
    }),
  },
  [EXPLAIN_STACK]: {
    prompt: () => ({
      imageSrc: 'images/explain/stack-spells.gif',
      text: 'explain stack',
      yesText: 'Intriguing...',
    }),
  },
  [EXPLAIN_WALK_ROPE]: {
    prompt: () => ({
      imageSrc: 'images/explain/walk-rope.gif',
      text: ['explain walk rope', keyToHumanReadable(keyMapping.showWalkRope)],
      yesText: 'Okay',
    }),
  },
  [EXPLAIN_END_TURN]: {
    prompt: () => ({
      imageSrc: 'images/explain/end-turn.gif',
      text: ['explain end turn', keyToHumanReadable(keyMapping.endTurn)],
      yesText: 'Okay',
    }),
  },
  [EXPLAIN_OVERFILL]: {
    condition: () =>
      !!globalThis.player &&
      globalThis.player.unit.mana > globalThis.player.unit.manaMax,
    prompt: () => ({
      imageSrc: 'images/explain/mana-overfill.gif',
      text: 'explain overfill',
      yesText: 'Cool!',
    }),
  },
  [EXPLAIN_MANA_COST]: {
    prompt: () => ({
      imageSrc: 'images/explain/mana-cost.gif',
      text: 'explain mana cost',
      yesText: 'Okay',
    }),
  },
  [EXPLAIN_ATTENTION_MARKER_MELEE]: {
    prompt: () => ({
      imageSrc: 'images/explain/attentionMarkerMelee.gif',
      text: 'explain attention markers melee',
      yesText: 'Okay',
    }),
  },
  [EXPLAIN_ATTENTION_MARKER_RANGED]: {
    prompt: () => ({
      imageSrc: 'images/explain/attentionMarkerRanged.gif',
      text: 'explain attention markers',
      yesText: 'Okay',
    }),
  },
  [EXPLAIN_CAMERA]: {
    prompt: () => ({
      imageSrc: 'images/explain/camera-movement.gif',
      text: [
        'explain camera movement',
        keyToHumanReadable(keyMapping.cameraUp),
        keyToHumanReadable(keyMapping.cameraLeft),
        keyToHumanReadable(keyMapping.cameraDown),
        keyToHumanReadable(keyMapping.cameraRight),
  ...
  [EXPLAIN_INVENTORY]: {
    prompt: () => ({
      imageSrc: 'images/explain/inventory.gif',
      text: ['explain inventory', keyToHumanReadable(keyMapping.openInventory)],
      yesText: "I'm so organized!",
    }),
  },
  [EXPLAIN_LIQUID_DAMAGE]: {
    prompt: () => ({
      imageSrc: 'images/explain/liquid-damage.gif',
      text: 'explain liquid damage',
      yesText: 'Yikes!',
    }),
  },
  [EXPLAIN_BLESSINGS]: {
    prompt: () => ({
      imageSrc: 'images/explain/bless-ally.gif',
      text: `explain blessings`,
      yesText: 'Got it!',
    }),
  },
  [EXPLAIN_REMOVE_SPELLS]: {
    prompt: () => ({
      imageSrc: 'images/explain/delete-queued-spells.gif',
      text: [
        'explain remove spells',
        keyToHumanReadable(keyMapping.dequeueSpell),
      ],
      yesText: 'Okay',
    }),
  },
  [EXPLAIN_FORGE_ORDER]: {
    prompt: () => ({
      imageSrc: 'images/explain/forge-order.gif',
      text: 'explain forge order',
      yesText: 'Okay',
    }),
  },
  [EXPLAIN_DEATH]: {
    prompt: () => ({
      imageSrc: 'images/explain/death.gif',
      text: 'explain death',
      yesText: 'Okay',
    }),
  },
  [EXPLAIN_MINI_BOSSES]: {
    prompt: () => ({
      imageSrc: 'images/explain/minibosses.gif',
      text: 'explain minibosses',
      yesText: 'Got it!',
    }),
  },
  [EXPLAIN_PING]: {
    prompt: () => ({
      imageSrc: 'images/explain/ping.gif',
      text: ['explain ping', keyToHumanReadable(keyMapping.ping)],
      yesText: 'Cool!',
    }),
  },
};
...
export function updateTutorialChecklist() {
  ...
  if (completeTutorialItems.length) {
    ...
    for (let item of completeTutorialItems) {
      if (item.visible) {
        completedItemsHtml += `<div class="complete">&#x2611; <span class="text complete">${i18n(
          item.text,
        )}</span></div>`;
      }
    }
    html += `<details>
            <summary>${i18n('Completed Tasks')} ${
      completeTutorialItems.length
    }/${tutorialItems.length}</summary>
            ${completedItemsHtml}
            </details>`;
  }
  for (let item of tutorialItems.filter((x) => !x.complete)) {
    if (item.visible) {
      html += `<div>&#x2610; <span class="text">${i18n(
        item.text,
      )}</span></div>`;
    }
  }
...
export function tutorialCompleteTask(
  key: keyof TutorialChecklist,
  condition?: () => boolean,
) {
  if (
    globalThis.doUpdateTutorialChecklist &&
...
globalThis.skipTutorial = async () => {
  const yesSkip = await Jprompt({
    text: 'skip tutorial detail',
    yesText: 'Yes',
    noBtnText: 'Cancel',
    noBtnKey: 'Escape',
    forceShow: true,
...
globalThis.resetTutorial = function resetTutorial() {
  ...
  Jprompt({
    text: 'Tutorial will reset after the game is restarted.',
    yesText: 'Okay',
    forceShow: true,
  });
};
...
export function isTutorialComplete() {
  ...
  const tutorialKeys: (keyof TutorialChecklist)[] = Object.keys(
    tutorialChecklist,
  ) as (keyof TutorialChecklist)[];

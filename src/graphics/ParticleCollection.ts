...
import {
  ...
  createHardCircleParticleTexture,
...
export function makeBloatExplosionWithParticles(
  position: Vec2,
  size: number,
  prediction: boolean,
) {
...
export function makeBleedParticles(
  position: Vec2,
  prediction: boolean,
  proportion: number,
  resolver?: () => void,
  ...
  simpleEmitter(
    { x: position.x, y: position.y - config.COLLISION_MESH_RADIUS / 2 },
    particleConfig,
    resolver,
  );
}
export function makeRisingParticles(
  position: Vec2,
  prediction: boolean,
  color: string = '#ffffff',
  emitterLifetime = 0.7,
...
export function makeBurstParticles(
  position: Vec2,
  finalScale: number,
  prediction: boolean,
  resolver?: () => void,
...
export function makeScrollDissapearParticles(
  position: Vec2,
  prediction: boolean,
) {
  if (prediction || globalThis.headless) {
...
export function makeDarkPriestAttackParticles(
  position: Vec2,
  prediction: boolean,
  resolver?: () => void,
) {
...
export function makeCorruptionParticles(
  follow: IUnit,
  prediction: boolean,
  ...
  const particleConfig = particles.upgradeConfig(cursedEmitterConfig(500), [
    texture,
  ]);
  if (containerUnits) {
...
export function makeCursedEmitter(position: Vec2, prediction: boolean) {
  ...
  const particleConfig = particles.upgradeConfig(cursedEmitterConfig(50), [
    texture,
  ]);
  return simpleEmitter(
    position,
...
export function makeDeathmasonPortal(
  position: Vec2,
  prediction: boolean,
  colorStart: string,
  colorEnd: string,

...
export const containerParticles = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.ParticleContainer(5000, {
      scale: true,
      position: true,
      rotation: false,
      uvs: false,
...
export const containerParticlesUnderUnits = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.ParticleContainer(5000, {
      scale: true,
      position: true,
...
export function wrappedEmitter(
  ...
  if (
    globalThis.emitters &&
    globalThis.limitParticleEmitters !== undefined &&
    globalThis.limitParticleEmitters !== -1 &&
    globalThis.emitters?.length >= globalThis.limitParticleEmitters
  ) {
    if (resolver) {
      resolver();
    }
...
export function simpleEmitter(
  ...
  const emitter: JEmitter = new particles.Emitter(
    container || containerParticles,
    config,
  );
...
// written by ChatGPT:
function createCurveTowardsFunction(
  start: Vec2,
  end: Vec2,
  control: Vec2,
): (t: number) => Vec2 {
  ...
  return raceTimeout(
    ...
    new Promise<void>((resolve) => {
      ...
      trails.push({
        lerp: 0,
        position,
        moveFn: createCurveTowardsFunction(position, target, control),
        emitter,
        resolver: resolve,
      });
    }),
...
export function updateParticlees(
  delta: number,
  ...
  for (var i = 0; i < bloods.length; i++) {
    ...
    var blood_size_mod =
      blood.scale < 1 ? randFloat(blood.scale, 1) : randFloat(1, blood.scale);
    const drawBloodPosition = {
      x: blood.x + blood_x_mod,
      y: blood.y + blood_y_mod,
    };

...
// PIXI app
export const app = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.Application();
export const containerLiquid = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.Container();
export const containerBoard = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.Container();
export const containerBloodSmear = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.Container();
export const containerRadiusUI = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.Container();
export const containerPlanningView = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.Container();
export const containerDoodads = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.Container();
export const containerUnits = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.Container();
export const containerSpells = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.Container();
export const containerProjectiles = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.Container();
export const containerUI = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.Container();
export const containerPlayerThinking = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.Container();
export const containerUIFixed = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.Container();
export const containerFloatingText = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.Container();

// Graphics used for painting blood trails
export const graphicsBloodSmear = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.Graphics();
// Container used for blood spatter particles
export const containerBloodParticles = !globalThis.pixi
  ? undefined
  : new globalThis.pixi.ParticleContainer();
...
function resetupBloodContainers() {
  tempBloodContainer = !globalThis.pixi
    ? undefined
    : new globalThis.pixi.Container();
  ...
  if (containerLiquid) {
    if (globalThis.pixi) {
      const displacementSprite =
        globalThis.pixi.Sprite.from('images/noise.png');
      displacementSprite.texture.baseTexture.wrapMode =
        globalThis.pixi.WRAP_MODES.REPEAT;

      const displacementFilter = new globalThis.pixi.filters.DisplacementFilter(
        displacementSprite,
      );
...
if (globalThis.pixi && containerUI && app && containerRadiusUI) {
  ...
  utilProps.elPIXIHolder = document.getElementById('PIXI-holder') as
    | HTMLElement
    | undefined;
  utilProps.elCardHoldersBorder = document.getElementById(
    'card-holders-border',
  ) as HTMLElement | undefined;
  utilProps.elCardHand = document.getElementById('card-hand') as
    | HTMLElement
    | undefined;
  utilProps.elCardHoldersInner = document.getElementById(
    'card-holders-inner',
  ) as HTMLElement | undefined;
...
function UIElementToInGameSpace(
  el: HTMLElement,
  pixiHolderRect: Rect,
  camX: number,
  camY: number,
...
export function withinCameraBounds(position: Vec2, marginHoriz?: number): Vec2 {
  ...
  const bottom =
    utilProps.elPIXIHolder.clientHeight / zoom - marginBottom + camY / zoom;
  ...
  const cardHoldersBorderBox = UIElementToInGameSpace(
    utilProps.elCardHoldersBorder,
    pixiHolderRect,
    camX,
    camY,
    zoom,
  ...
  if (
    isWithinRect(
      { x: withinBoundsPos.x - margin, y: withinBoundsPos.y },
      cardHoldersBorderBox,
    ) ||
    isWithinRect(
      { x: withinBoundsPos.x + margin, y: withinBoundsPos.y },
      cardHoldersBorderBox,
...
export function runCinematicLevelCamera(underworld: Underworld): Promise<void> {
  ...
  return new Promise<void>((resolve) => {
    ...
    const margin = 3;
    const firstUnitToView = clampCameraPosition(
      { x: underworld.limits.xMin, y: underworld.limits.yMin },
      realCam.zoom,
      underworld,
    ); //positionSortedUnits[0];
    const lastUnitToView = clampCameraPosition(
      { x: underworld.limits.xMax, y: underworld.limits.yMax },
      realCam.zoom,
      underworld,
    ); //positionSortedUnits[positionSortedUnits.length - 1];
    if (
      !firstUnitToView ||
      isInvalid(firstUnitToView) ||
      !lastUnitToView ||
      isInvalid(lastUnitToView)
    ) {
    ...
    let cinematicCam: {
      ...
      radius:
        Math.min(elPIXIHolder.clientWidth, elPIXIHolder.clientHeight) /
        realCam.zoom /
        margin,
      x: firstUnitToView.x,
      y: firstUnitToView.y,
    ...
    const loop = (timestamp: number) => {
      ...
      const stepTowardsTarget = math.getCoordsAtDistanceTowardsTarget(
        cinematicCam,
        cinematicCam.target,
        cinematicCameraMoveSpeed * elapsed,
      );
      ...
      if (
        ...
        math.distance(cinematicCam, mapCenter) < 1
      ) {
      ...
      if (
        cinematicCam.target !== mapCenter &&
        math.distance(cinematicCam, cinematicCam.target) <= cinematicCam.radius
      ) {
...
export function tryShowRecenterTip() {
  if (!utilProps.doCameraAutoFollow && elCameraRecenterTip) {
    if (globalThis.player?.isSpawned) {
      ...
      const screenRadiusInGameSpace =
        Math.min(elPIXIHolder.clientWidth, elPIXIHolder.clientHeight) /
        zoom /
        2;
      const margin = config.COLLISION_MESH_RADIUS * 2;
      const cameraGameSpaceCenterDistanceToPlayer = math.distance(
        getCameraCenterInGameSpace(),
        globalThis.player.unit,
      );
      // Only show recenter tip if player is near or beyond the edge of the screen
      if (
        cameraGameSpaceCenterDistanceToPlayer + margin >
        screenRadiusInGameSpace
      ) {
        elCameraRecenterTip.innerHTML = i18n([
          'Press 🍞 to make the view auto follow you',
          keyToHumanReadable(KeyMapping.recenterCamera),
        ]);
      } else {
...
export function getMapCenter(underworld: Underworld): Vec2 {
  return {
    x: (underworld.limits.xMax - underworld.limits.xMin) / 2,
    y: (underworld.limits.yMax - underworld.limits.yMin) / 2,
  };
}
  ...
  switch (globalThis.view) {
    case View.Game:
      if (globalThis.player) {
        ...
        if (utilProps.doCameraAutoFollow) {
          utilProps.camera = clampCameraPosition(
            utilProps.camera,
            zoom,
            underworld,
          );
...
function clampCameraPosition(
  ...
  const camCenterYMax =
    mapBottomMostPoint - elPIXIHolder.clientHeight / 2 / zoom;
  // If the supposed minimum is more than the maximum, just center the camera:
...
export function updateNameText(nameText?: PIXI.Text, zoom?: number) {
  if (nameText) {
    ...
    if (zoom) {
      ...
      nameText.y =
        -config.COLLISION_MESH_RADIUS - config.NAME_TEXT_Y_OFFSET / zoom;
    }
  ...
  return new Promise((resolve, reject) => {
    if (!globalThis.headless && globalThis.pixi) {
      ...
      loader.onComplete.add((loader, resources) => {
        const sheetPaths = Object.keys(resources).filter((path) =>
          path.endsWith('.json'),
        );
        for (let sheetPath of sheetPaths) {
          ...
          if (
            ...
            sheets.indexOf(resource.spritesheet) === -1
          ) {
...
export function getPixiTextureAnimated(imagePath: string) {
  // Headless does not use graphics
  if (globalThis.headless) {
    return;
  }
...
export function addPixiSpriteAnimated(
  ...
  if (texture) {
    ...
    animatedSprite.animationSpeed =
      options.animationSpeed || config.DEFAULT_ANIMATION_SPEED;
    ...
    if (options.colorReplace) {
      const robeMagicColorFilter = new MultiColorReplaceFilter(
        options.colorReplace.colors,
        options.colorReplace.epsilon,
      );
    ...
    throw new Error('Could not find animated texture for ' + imagePath);
  }
}
...
export function addPixiTilingSprite(
  ...
  if (!singleTexture) {
    console.error(
      ...
      imagePath,
      'check the spritesheet to figure out why it is missing.',
    );
    return undefined;
  }
...
export function pixiText(
  ...
  const textSprite = new globalThis.pixi.Text(text, {
    fontFamily: 'Forum',
    ...style,
  });
...
export function startBloodParticleSplatter(
  underworld: Underworld,
  damageOrigin: Vec2,
  target: IUnit,
  options?: { maxRotationOffset: number; numberOfParticles: number },
) {
  if (globalThis.headless) {
    return;
  }
  ...
  for (let i = 0; i < bloodAmount; i++) {
    ...
    const MAX_ROTATION_OFFSET = options
      ? options.maxRotationOffset
      : Math.PI / 4;
    ...
    const randRotationOffset = isDamageFromSelf
      ? randFloat(-Math.PI, Math.PI)
      : randFloat(-MAX_ROTATION_OFFSET, MAX_ROTATION_OFFSET);
    ...
    const proportionOfMaxAwayFromCenterLine = Math.abs(
      randRotationOffset / MAX_ROTATION_OFFSET,
    );
    ...
    const speed = Math.min(
      2,
      DISTANCE_MAGNIFIER * Math.abs(1 - proportionOfMaxAwayFromCenterLine),
    );

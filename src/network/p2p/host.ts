...
export async function host({fromName, websocketHubUrl, onPeerConnected, onPeerDisconnected, onError}:{fromName: string, websocketHubUrl: string, onPeerConnected: (p:SimplePeer)=>void, onPeerDisconnected: (p:SimplePeer) => void, onError: (error:any)=>void}) {
    function onData(data: any, socket: WebSocket) {
    ...
    const socket = await ensureConnectionToHub(websocketHubUrl, { onData, onError });

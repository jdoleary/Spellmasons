...
export function onClientPresenceChanged(
  o: ClientPresenceChangedArgs,
  overworld: Overworld,
) {
...
export function hostGiveClientGameState(
  clientId: string,
  underworld: Underworld,
  level: LevelData | undefined,
  message_type: MESSAGE_TYPES.INIT_GAME_STATE | MESSAGE_TYPES.LOAD_GAME_STATE,
) {
  ...
  if (globalThis.isHost(underworld.pie)) {
    ...
    if (globalThis.clientId !== clientId) {
      if (level) {
        ...
        console.error(
          'hostGiveClientGameState: Could not send INIT_GAME_STATE, levelData is undefined',
        );
      }
    }
  }
...
export function getVersionInequality(
  clientVersion?: string,
  serverVersion?: string,
): 'equal' | 'client behind' | 'server behind' | 'malformed' {
  if (clientVersion && serverVersion) {
    ...
    if (
      clientMajor !== undefined &&
      clientMinor !== undefined &&
      ...
      if (clientMajor !== serverMajor || clientMinor !== serverMinor) {
        if (parseInt(serverMajor) > parseInt(clientMajor)) {
          ...
          return parseInt(serverMinor) > parseInt(clientMinor)
            ? 'client behind'
            : 'server behind';
        }
      } else {

...
export function calculateGameDifficulty(underworld: Underworld) {
  const loopDifficultyModifier =
    0.2 * Math.max(0, underworld.levelIndex - config.LAST_LEVEL_INDEX);
  let difficulty =
    underworld.players.filter((p) => p.clientConnected).length +
    loopDifficultyModifier;
...
export function unavailableUntilLevelIndexDifficultyModifier(

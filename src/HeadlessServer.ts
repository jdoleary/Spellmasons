...
const pie = isUsingBun
  ? require('@websocketpie/server-bun')
  : require('@websocketpie/server');
  ...
  pie.startServer({
    ...
    makeHostAppInstance: () => {
      ...
      if (hostAppInst.overworld.underworld) {
        ...
        hostAppInst.overworld.underworld.lastLevelCreated =
          hostAppInst.overworld.underworld.generateLevelDataSyncronous(0);
        // Actually create the level
        hostAppInst.overworld.underworld.createLevelSyncronous(
          hostAppInst.overworld.underworld.lastLevelCreated,
        );
      } else {
        console.error(
          'hostApp overworld does not have an underworld to initialize.',
        );
      }
      return hostAppInst;
    },

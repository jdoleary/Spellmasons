...
export const sfx: { [key: string]: string[] } = {
  ...
  reroll: [
    './sound/sfx/Reroll_v2-001.wav',
    './sound/sfx/Reroll_v2-002.wav',
    './sound/sfx/Reroll_v2-003.wav',
  ],
  ...
  summonerSummon: ['./sound/sfx/summoner-summon.mp3'],
  swap: ['./sound/sfx/swap.mp3'],
  targetAquired: [
    './sound/sfx/Target_variation-001.mp3',
...
export async function playNextSong() {
  ...
  if (musicInstance) {
    ...
    await Promise.race([
      new Promise((resolve) =>
        setTimeout(resolve, MILLIS_TO_FADE_OUT_SONG + 100),
      ),
  ...
  musicInstance.volume =
    (globalThis.volume === undefined ? 1 : globalThis.volume) *
    (globalThis.volumeMusic === undefined ? 1 : globalThis.volumeMusic);
  try {
    musicInstance.play();
  } catch (e) {
...
export function playSFX(path?: string) {
  ...
  if (
    lastTimeThisPathWasPlayed &&
    Date.now() - lastTimeThisPathWasPlayed <= 100
  ) {
  ...
  audioInstance.volume =
    (globalThis.volume === undefined ? 1 : globalThis.volume) *
    (globalThis.volumeGame === undefined ? 1 : globalThis.volumeGame);
  audioInstance.play();
}
...
export function setupAudio() {
  ...
  globalThis.changeVolumeMusic = (
    volume: number,
    saveSetting: boolean = true,
  ) => {
    globalThis.volumeMusic = volume;
    if (saveSetting) {
      storage.assign(storage.STORAGE_OPTIONS, {
        volumeMusic: globalThis.volumeMusic,
      });
    }
    if (musicInstance) {
      musicInstance.volume =
        (globalThis.volume === undefined ? 1 : globalThis.volume) *
        globalThis.volumeMusic;
    }
    playMusicIfNotAlreadyPlaying();
  };
  globalThis.changeVolumeGame = (
    volume: number,
    saveSetting: boolean = true,
  ) => {
    ...
    if (saveSetting) {
      ...
      demoSoundWhenChangingVolume();
      storage.assign(storage.STORAGE_OPTIONS, {
        volumeGame: globalThis.volumeGame,
      });
    }
  };
}

...
export function getSavedData() {
  ...
  (globalThis.isElectron && globalThis.diskStorage
    ...
    if (!color) {
      const newColor =
        robeColors[Math.floor(Math.random() * robeColors.length)] || 0xef476f;
    ...
    if (globalThis.isElectron) {
      if (globalThis.electronSettings) {
        ...
        console.error(
          'globalThis.electronSettings is undefined, cannot set uiZoom',
        );
      }
    }
    // Update controls:
    ...
    globalThis.enemyEncountered = JSON.parse(
      get(ENEMY_ENCOUNTERED_STORAGE_KEY) || '[]',
    );
    console.log(
      'Setup: initializing enemyEncountered as',
      globalThis.enemyEncountered,
    );
    // Update spells discovered:
    globalThis.spellsDiscovered = JSON.parse(
      get(SPELLS_DISCOVERED_STORAGE_KEY) || '[]',
    );
    console.log(
      'Setup: initializing spellsDiscovered as',
      globalThis.spellsDiscovered,
    );
  });
}
...
export function get(key: string): string | null {
  ...
  if (globalThis.allowCookies || areCookiesAllowed()) {
    ...
    console.log(
      `Could not retrieve "${key}" from storage, without cookie consent`,
    );
    return null;
  }
}
...
globalThis.setCinematicCameraEnabled = (
  enabled: boolean,

...
function intersectionOfLines(
  line: LineInStandardForm,
  line2: LineInStandardForm,
): Vec.Vec2 {
  // https://www.cuemath.com/geometry/intersection-of-two-lines/
  return {
    x:
      (line.b * line2.c - line2.b * line.c) /
      (line.a * line2.b - line2.a * line.b),
    y:
      (line.c * line2.a - line2.c * line.a) /
      (line.a * line2.b - line2.a * line.b),
  };
}
...
function findWherePointIntersectLineAtRightAngle(
  point: Vec.Vec2,
  line: LineInStandardForm,
): Vec.Vec2 {
  ...
  const line2 = {
    p1: point,
    p2: { x: point.x + inverseLine1Slope, y: point.y + 1 },
  };
...
export function findWherePointIntersectLineSegmentAtRightAngle(
  point: Vec.Vec2,
  line: LineSegment,
): Vec.Vec2 | undefined {
  ...
  if (lineInStandardForm) {
    const intersection = findWherePointIntersectLineAtRightAngle(
      point,
      lineInStandardForm,
    );
    // Return the intersection point IF it is between the endpoints of the line segment
    if (
      intersection.y <= largestY &&
      intersection.y >= smallestY &&
      intersection.x <= largestX &&
      intersection.x >= smallestX
    ) {
      return intersection;
    } else {
...
export function isPointOnLineSegment(
  ...
  if (segmentSlope == slopeFromPointToEndOfSegment) {
    const pointIsInBoundingBoxOfSegment =
      point.x >= Math.min(lineSegment.p1.x, lineSegment.p2.x) &&
...
export function getParametricRelation(
  l1: LineSegment,
  l2: LineSegment,
): ParametricRelation {
  ...
  if (isCollinear) {
    if (!pointInSameDirection) {
      ...
      const reversedRelation = getParametricRelation(l1, {
        p1: l2.p2,
        p2: l2.p1,
      });
      return {
      ...
      const isOverlapping =
        l2p1Insidel1 || l2p2Insidel1 || l2FullyCoversl1 || l1FullyCoversl2;
      return {
        p,
        r,
        q,
...
export function getRelation(
  ...
  const { isCollinear, pointInSameDirection, isOverlapping } =
    getParametricRelation(l1, l2);
...
export function isCollinearAndPointInSameDirection(
  l1: LineSegment,
  l2: LineSegment,
): boolean {
...
export function isCollinearAndOverlapping(
  l1: LineSegment,
  l2: LineSegment,
): boolean {
...
export function closestLineSegmentIntersectionWithLine(
  l1: LineSegment,
  otherLines: LineSegment[],
): { intersection: Vec.Vec2; lineSegment: LineSegment } | undefined {
  ...
  return closestIntersection && lineThatIntersected
    ? { intersection: closestIntersection, lineSegment: lineThatIntersected }
    : undefined;
}
// Test l1 for intersections with each of otherLines; of all the intersections return the closest intersection
export function closestLineSegmentIntersection(
  l1: LineSegment,
  otherLines: LineSegment[],
): Vec.Vec2 | undefined {
  ...
  if (isCollinear) {
    if (isOverlapping) {
      ...
      if (l2p1Insidel1 && l2p2Insidel1) {
        ...
        console.error(
          'lineSegmentIntersection: this line should be impossible to reach',
        );

...
import {
  ...
  Polygon2,
  Polygon2LineSegment,
  doesVertexBelongToPolygon,
  getInsideAnglesOfWall,
  doesLineFromPointToTargetProjectAwayFromOwnPolygon,
  getPointsFromPolygonStartingAt,
} from './Polygon2';

// Will return either an array with 1 normal polygon or an array with 1 or more
// inverted polygons
export function findPolygonsThatVec2IsInsideOf(
  point: Vec2,
  testPolygons: Polygon2[],
): Polygon2[] {
...
export function findPath(
  ...
  for (let i = 0; i < processingLimit; i++) {
    ...
    const shortestFinishedPaths = paths
      .filter((p) => p.done && !p.invalid)
    ...
    function tryOptimizePath(path: Path): boolean {
      for (let i = 0; i < path.points.length; i++) {
        for (let j = path.points.length - 1; j > i + 1; j--) {
          ...
          let { intersectingWall, closestIntersection } =
            getClosestIntersectionWithWalls(nextLine, pathingLineSegments);
          if (intersectingWall) {
            const intersectingPoly = intersectingWall.polygon;

            const indexIfP2IsOnVertex = intersectingPoly.findIndex((p) =>
              Vec.equal(p, nextLine.p2),
            );
            if (indexIfP2IsOnVertex !== -1) {
              ...
              const lineIsCastIntoNoWalkZone = isAngleBetweenAngles(
                Vec.getAngleBetweenVec2s(nextLine.p2, nextLine.p1),
                insideAngleOfWall.start,
                insideAngleOfWall.end,
              );
  ...
  if (
    !shortestPath &&
    firstIntersection &&
    Vec.equal(startPoint, firstIntersection)
  ) {
    return [];
  }
  return shortestPath
...
// Note: Mutates Path
function addWalkAroundPolyInfoToPath(
  path: Path,
  direction: 'prev' | 'next',
  startVertex: Vec2,
  poly: Polygon2,
...
function walkAroundAPoly(path: Path, pathingWalls: Polygon2LineSegment[]) {
  ...
  if (
    endPoint &&
    isPointOnLineSegment(path.target, { p1: endPoint, p2: vertex })
  ) {
    path.complete = true;
    path.done = true;
    return;
  ...
  const indexOfVertex = path.walkAroundPolyInfo.poly.findIndex((p) =>
    Vec.equal(p, vertex),
  );
  // true if line to target doesn't pass through own polygon
  let lineToTargetDoesNotPassThroughOwnPolygon = false;
  if (indexOfVertex >= 0) {
    lineToTargetDoesNotPassThroughOwnPolygon =
      doesLineFromPointToTargetProjectAwayFromOwnPolygon(
        path.walkAroundPolyInfo.poly,
        indexOfVertex,
        path.target,
      );
  }
  ...
  if (lineToTargetDoesNotPassThroughOwnPolygon) {
    ...
    const { intersectingWall, closestIntersection } =
      getClosestIntersectionWithWalls(
        { p1: vertex, p2: path.target },
        pathingWalls,
      );
    // If it does
    if (intersectingWall && closestIntersection) {
      ...
      if (
        doesVertexBelongToPolygon(vertex, intersectingWall.polygon) &&
        doesVertexBelongToPolygon(intersectingWall.p1, intersectingWall.polygon)
      ) {
        ...
        if (
          path.points.find((p) => p == intersectingWall.p1) &&
          path.points.find((p) => p == intersectingWall.p2)
        ) {
...
function processPaths(
  paths: Path[],
  pathingWalls: Polygon2LineSegment[],
): Path[] {
  // Continue to process paths that are incomplete
  tryAllPaths: for (let path of paths) {
    ...
    if (path.walkAroundPolyInfo) {
      ...
      if (intersectingWall && closestIntersection) {
        if (Vec.equal(closestIntersection, nextStraightLine.p2)) {
          ...
          checkPaths: for (let otherPath of paths) {
            ...
            if (otherPath !== path) {
              for (let i = 0; i < otherPath.points.length; i++) {
                ...
                if (point && Vec.equal(closestIntersection, point)) {
                  const lengthOfCurrentPathToThisVertex =
                    calculateDistanceOfVec2Array([
                      ...path.points,
                      closestIntersection,
                    ]);
                  const lengthOfOtherPathToThisVertex =
                    calculateDistanceOfVec2Array(
                      otherPath.points.slice(0, i + 1),
                    );
                  if (
                    lengthOfCurrentPathToThisVertex <
                    lengthOfOtherPathToThisVertex
                  ) {
          ...
          const branchedPath = {
            ...path,
            points: path.points.map((p) => Vec.clone(p)),
          };
          ...
          addWalkAroundPolyInfoToPath(
            path,
            'prev',
            nextWalkPoint,
            intersectingWall.polygon,
          );
...
export function removeBetweenIndexAtoB(
  array: any[],
  indexA: number,
  indexB: number,
): any[] {
...
function polygonLineSegmentToPrevAndNext(wall: Polygon2LineSegment): {
  prev: Vec2;
  next: Vec2;
} {
...
function getClosestIntersectionWithWalls(
  line: LineSegment,
  walls: Polygon2LineSegment[],
  includeStartPoint: boolean = false,
): { intersectingWall?: Polygon2LineSegment; closestIntersection?: Vec2 } {
    ...
    if (intersection) {
      ...
      if (Vec.equal(line.p1, intersection)) {
        if (!includeStartPoint) {
          ...
          const lineIsCastIntoNoWalkZone = isAngleBetweenAngles(
            Vec.getAngleBetweenVec2s(line.p1, line.p2),
            insideAngleOfWall.start,
            insideAngleOfWall.end,
          );
          if (!lineIsCastIntoNoWalkZone) {
...
export function pointsEveryXDistanceAlongPath(
  startPoint: Vec2,
  path: Vec2[],
  distanceOfIncrements: number,
  alreadyUsedDistance: number = 0,
): Vec2[] {
  ...
  for (let point of path) {
    ...
    if (nextDistance > distanceToPoint) {
      ...
      const numberOfPointsOnThisLine = Math.ceil(
        distanceToPoint / distanceOfIncrements,
      );

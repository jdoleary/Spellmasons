...
export function getPointNormalVector(
  point: Vec2,
  prevPoint: Vec2,
  nextPoint: Vec2,
): Vec2 {
...
function getOutsideAngleOfPoint(
  prevPoint: Vec2,
  point: Vec2,
  nextPoint: Vec2,
): number {
...
export type Polygon2LineSegment =
  LineSegment.LineSegment & // The polygon that these points belong to
  { polygon: Polygon2 };
...
export function mergeCollinearOverlappingSameDirectionLines(
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    if (line) {
      const linesForMerging = lines.filter((l) => {
        ...
        return (
          relation.isCollinear &&
          relation.isOverlapping &&
          relation.pointInSameDirection
        );
      });
      ...
      if (newLine) {
        for (let mergeLine of linesForMerging) {
          ...
          if (
            distance(mergeLine.p1, newLine.p2) >
            distance(newLine.p1, newLine.p2)
          ) {
            newLine.p1 = mergeLine.p1;
          }
          if (
            distance(newLine.p1, mergeLine.p2) >
            distance(newLine.p1, newLine.p2)
          ) {
...
export function splitIntersectingLineSegments(
  ...
  for (let other of lineSegments) {
    ...
    if (intersection) {
      ...
      if (
        !Vec.equal(intersection, line.p1) &&
        !Vec.equal(intersection, line.p2)
      ) {
...
export function splitIntersectingPolygon2LineSegments(
  lineSegments: Polygon2LineSegment[],
): Polygon2LineSegment[] {
  ...
  for (let line of lineSegments) {
    ...
    splitPolygon2LineSegments.push(
      ...splitLineSegments.map((ls) => ({ ...ls, polygon: line.polygon })),
    );
  }
...
export function mergePolygon2s(polygons: Polygon2[]): Polygon2[] {
  ...
  for (let i = polyLineSegments.length - 1; i >= 0; i--) {
    ...
    if (lineSegment) {
      ...
      for (let poly of polygons) {
        ...
        if (
          toLineSegments(poly).filter(
            (ls) =>
              LineSegment.isPointOnLineSegment(lineSegment.p1, ls) ||
              LineSegment.isPointOnLineSegment(lineSegment.p2, ls),
          ).length == 0
        ) {
          continue;
        }
        ...
        if (isInside) {
          const notDirectlyOnLine = toLineSegments(poly).every(
            (ls) => !LineSegment.isPointOnLineSegment(center, ls),
          );
  ...
  for (let line of polyLineSegments) {
    reversals.push(
      ...polyLineSegments.filter(
        (other) => Vec.equal(line.p1, other.p2) && Vec.equal(line.p2, other.p1),
      ),
    );
  }
  polyLineSegments = polyLineSegments.filter(
    (line) => !reversals.includes(line),
  );
  // Remove unnecessary in-between verticies:
  const lineSegments =
    mergeCollinearOverlappingSameDirectionLines(polyLineSegments);
...
export function growOverlappingCollinearLinesInDirectionOfP2(
  line: LineSegment.LineSegment,
  ...
  let relevantWalls = walls.filter((w) =>
    LineSegment.isCollinearAndPointInSameDirection(line, w),
  );
  ...
  for (let i = 0; i < originalNumberOfPotentialGrowthLoops; i++) {
    ...
    for (let wall of relevantWalls) {
      if (
        LineSegment.isCollinearAndOverlapping(line, wall) &&
        distance(line.p1, line.p2) < distance(line.p1, wall.p2)
      ) {
        testLineGrew = true;
...
export function processLineSegment(
  processingLineSegment: LineSegment.LineSegment,
  lineSegments: LineSegment.LineSegment[],
): Polygon2 {
  ...
  do {
    ...
    const branch = getClosestBranch(currentLine, [
      ...lineSegments,
      ...danglingLineSegments,
      ...usedLineSegments,
    ]);
    if (branch === undefined) {
      if (lastMatch) {
        ...
        const matches = newPoly.map(
          (p) => lastMatch && Vec.equal(p, lastMatch),
        );
      ...
      const indexOfBranchIntersectionInPoly = newPoly.findIndex((p) =>
        Vec.equal(branch.intersection, p),
      );
      if (indexOfBranchIntersectionInPoly !== -1) {
        if (
          lastMatch &&
          lastIntersection &&
          Vec.equal(lastMatch, lastIntersection)
        ) {
    ...
    if (Vec.equal(branch.intersection, branch.branchingLine.p2)) {
      console.error(
        'Unexpected: intersection should not be equal to branchingLine.p2',
      );
    }
    if (Vec.equal(branch.intersection, currentLine.p1)) {
      console.error(
        'Unexpected: intersection should not be equal to currentLine.p1',
      );
    }
    ...
    const branchingLineIndex = lineSegments.findIndex(
      (ls) => ls == branch.branchingLine,
    );
    ...
    if (!Vec.equal(branch.intersection, currentLine.p2)) {
      danglingLineSegments.push({
        p1: branch.intersection,
        p2: currentLine.p2,
      });
    }
...
export function toPolygon2LineSegments(
  polygon: Polygon2,
): Polygon2LineSegment[] {
...
export function isVec2InsidePolygon(point: Vec2, polygon: Polygon2): boolean {
  ...
  const horizontalLine: LineSegment.LineSegment = {
    p1: point,
    p2: { x: Number.MAX_SAFE_INTEGER, y: point.y },
  };
  // Start outside, so each odd number of flips will determine it to be inside
  let isInside = false;
  const intersections: Vec2[] = [];
  for (let wall of toLineSegments(polygon)) {
    const _intersection = LineSegment.lineSegmentIntersection(
      horizontalLine,
      wall,
    );
    ...
    const intersection = _intersection
      ? { x: +_intersection.x.toFixed(2), y: +_intersection.y.toFixed(2) }
      : undefined;
    ...
    if (
      ...
      if (Vec.equal(intersection, point)) {
        ...
        Vec.equal(intersection, wall.p1) ||
        Vec.equal(intersection, wall.p2)
      ) {
        // Get the INSIDE angle of the vertex (relative to it's polygon)
        const indexOfVertex = polygon.findIndex((p) =>
          Vec.equal(p, intersection),
        );
        ...
        if (nextPoint && prevPoint) {
          const startClockwiseAngle = Vec.getAngleBetweenVec2s(
            intersection,
            nextPoint,
          );
          const endClockwiseAngle = Vec.getAngleBetweenVec2s(
            intersection,
            prevPoint,
          );
          // Take the vectors: line.p1 (the point) to vertex/intersection and vertex/intersection to line.p2
          const v1Angle = Vec.getAngleBetweenVec2s(
            intersection,
            horizontalLine.p1,
          );
          const v2Angle = Vec.getAngleBetweenVec2s(
            intersection,
            horizontalLine.p2,
          );
          const allowableAngle = clockwiseAngle(
            startClockwiseAngle,
            endClockwiseAngle,
          );
          const v1AngleInside =
            Vec.equal(intersection, point) ||
            clockwiseAngle(startClockwiseAngle, v1Angle) <= allowableAngle;
          const v2AngleInside =
            clockwiseAngle(startClockwiseAngle, v2Angle) <= allowableAngle;
          ...
          console.error(
            'Next point or prev point is undefined. This error should never occur.',
          );
        }
      } else {
...
function projectPointForPathingMesh(
  polygon: Polygon2,
  pointIndex: number,
  magnitude: number,
): Vec2 {
  ...
  if (point) {
    ...
    if (nextPoint && prevPoint) {
      ...
      console.error(
        'projectPointForPathingMesh: nextPoint or prevPoint is undefined.  This error should never happen.',
      );
      return { x: 0, y: 0 };
    }
  } else {
    console.error(
      'projectPointForPathingMesh: point is undefined.  This error should never happen.',
    );
    return { x: 0, y: 0 };
  }
}
...
export function expandPolygon(polygon: Polygon2, magnitude: number): Polygon2 {
  return polygon.map((_p, i) =>
    projectPointForPathingMesh(polygon, i, magnitude),
  );
}

// Refactored from Polygon.ts
export function* makePolygonIndexIterator(
  polygon: Polygon2,
  startIndex: number = 0,
...
// Refactored from Polygon.ts
export function getPointsFromPolygonStartingAt(
  polygon: Polygon2,
  startPoint: Vec2,
): Vec2[] {
  ...
  if (startPointIndex == -1) {
    ...
    const polygonIndicies = Array.from(
      makePolygonIndexIterator(polygon, startPointIndex),
    );
...
// Refactored from Polygon.ts
export function getInsideAnglesOfWall(p: Polygon2LineSegment): {
  start: number;
  end: number;
} {
...
// part of the polygon
export function getInsideAnglesOfPoint(
  polygon: Polygon2,
  pointIndex: number,
  ...
  if (point) {
    ...
    if (nextPoint && prevPoint) {
      ...
      console.error(
        'getInsideAnglesOfPoint: nextPoint or prevPoint is undefined. This error should never happen.',
      );
      return { start: 0, end: 0 };
    }
  } else {
    console.error(
      'getInsideAnglesOfPoint: point is undefined. This error should never happen.',
    );
    return { start: 0, end: 0 };
  }
}
...
export function doesLineFromPointToTargetProjectAwayFromOwnPolygon(
  polygon: Polygon2,
  pointIndex: number,
  target: Vec2,
): boolean {

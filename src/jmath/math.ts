...
export function getCoordsAtDistanceTowardsTarget(
  start: Vec2,
  target: Vec2,
  travelDist: number,
  allowMoveBeyondTarget: boolean = false,
): Vec2 {
...
export function getPosAtAngleAndDistance(
  origin: Vec2,
  radians: number,
  distance: number,
): Vec2 {
...
export function* honeycombGenerator(
  radius: number,
  start: Vec2,
  loopLimit: number,
): Generator<Vec2> {

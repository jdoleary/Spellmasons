...
export function randInt(
  ...
  return Math.floor(
    randFloat(minInclusive, maxInclusive + 1, seedrandomInstance),
  );
}
...
export function _chooseObjectWithProbability<T extends objectWithProbability>(
  roll: number,
  source: T[],
): T | undefined {
  ...
  for (let x of source) {
    if (roll > rollingLowerBound && roll <= x.probability + rollingLowerBound) {
      return x;
    } else {
...
export function chooseOneOfSeeded<T>(
  arr: T[],
  seedRandomInstance: prng,
): T | undefined {
...
export function getUniqueSeedString(
  ...
  const playerUniqueIdentifier = !player
    ? '0'

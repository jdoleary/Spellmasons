...
export function isVecIntersectingVecWithCustomRadius(
  c1: Vec2,
  c2: Vec2,
  radius: number,
): boolean {
...
// Pure Function
export function moveAlongVector(
  startPos: Vec2,
  normalizedVector: Vec2,
  distance: number,
): Vec2 {
  return {
...
export function normalizedVector(
  point1: Vec2,
  point2: Vec2,
): { vector: Vec2 | undefined; distance: number } {
...
export function collideWithLineSegments(
  circle: Circle,
...
// moving slower
export function forceMovePreventForceThroughWall(
  forceMoveInst: ForceMove,
  underworld: Underworld,
): boolean {
  ...
  if (magnitude(velocity) >= pushedObject.radius) {
    for (let wall of underworld.walls) {
      const intersection = lineSegmentIntersection(
        { p1: pushedObject, p2: add(pushedObject, velocity) },
        wall,
      );
      if (intersection) {
        const newPos = math.getCoordsAtDistanceTowardsTarget(
          intersection,
          pushedObject,
          pushedObject.radius,
        );
    ...
    if (overlap > 0) {
      if (otherIsFixed) {
        ...
        const intersection = closestLineSegmentIntersection(
          { p1: mover, p2: moverPos },
          underworld.pathingLineSegments,
        );
        if (intersection) {
...
function repelCircleFromLine(
  ...
  const totalRepelDistance =
    config.COLLISION_MESH_RADIUS * config.NON_HEAVY_UNIT_SCALE;
  ...
  const rightAngleIntersectionWithLineFromMoverCenterPoint =
    findWherePointIntersectLineSegmentAtRightAngle(mover, line);
  if (
    rightAngleIntersectionWithLineFromMoverCenterPoint &&
    distance(rightAngleIntersectionWithLineFromMoverCenterPoint, mover) <=
      totalRepelDistance
  ) {
    ...
    const repelVector = subtract(
      mover,
      rightAngleIntersectionWithLineFromMoverCenterPoint,
    );

    const newLocationRelative = similarTriangles(
      repelVector.x,
      repelVector.y,
      magnitude(repelVector),
      totalRepelDistance,
    );
    const newLocation = add(
      rightAngleIntersectionWithLineFromMoverCenterPoint,
      newLocationRelative,
    );
  ...
  if (distance(line.p1, mover) <= totalRepelDistance) {
    ...
    repelCircles(
      mover,
      mover,
      { ...line.p1, radius: totalRepelDistance - mover.radius },
      underworld,
      true,
    );
    repelled = true;
  ...
  if (distance(line.p2, mover) <= totalRepelDistance) {
    ...
    repelCircles(
      mover,
      mover,
      { ...line.p2, radius: totalRepelDistance - mover.radius },
      underworld,
      true,
    );
    repelled = true;

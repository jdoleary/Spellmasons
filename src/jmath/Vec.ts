...
export function getAngleBetweenVec2sYInverted(
  origin: Vec2,
  point: Vec2,
): number {
  const dy = point.y - origin.y;
...
export function dotProduct(p1: Vec2, p2: Vec2): number {
  ...
  const angle = clockwiseAngle(
    getAngleBetweenVec2s(origin, p1),
    getAngleBetweenVec2s(origin, p2),
  );
...
// a given magnitude
export function getEndpointOfMagnitudeAlongVector(
  pos: Vec2,
  angle: number,
  magnitude: number,
): Vec2 {
  const nextPointDirection = {
  ...
  return add(
    pos,
    similarTriangles(
      nextPointDirection.x - pos.x,
      nextPointDirection.y - pos.y,
      dist,
      magnitude,
    ),
  );

  ...
  if (location && location.href.includes('localhost')) {
    ...
    for (let upgrade of all_upgrades) {
      if (all_upgrades.filter((u) => u.title == upgrade.title).length > 1) {
        console.error(
          `Multiple upgrades with the same title "${upgrade.title}"`,
        );
      }
    }
  }
...
export function ensureAllClientsHaveAssociatedPlayers(
  overworld: Overworld,
  clients: string[],
  defaultLobbyReady?: boolean,
) {
  if (!overworld) {
...
export function recalculateGameDifficulty(underworld: Underworld) {
  ...
  console.log(
    'adjusting game difficulty to ',
    newDifficulty,
    ' for ',
    underworld.players.filter((p) => p.clientConnected).length,
    ' connected players.',
  );
}

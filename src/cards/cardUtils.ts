...
export function levelsUntilCardIsEnabled(
  cardId: string,
...
export function getCardCooldown(
  cardId: string,
  underworld?: Underworld,
): number {
  if (!globalThis.player || !underworld) {
...
export async function playDefaultSpellAnimation(
  card: ICard,
  targets: Vec2[],
  prediction: boolean,
) {
...
export async function animateSpell(
  target: Vec2,
  imagePath: string,
): Promise<void> {
  if (globalThis.headless) {
  ...
  if (imagePath.indexOf('.png') !== -1) {
    console.error(
      `Cannot animate a still image ${imagePath}, this function requires an animation path or else it will not "hide when complete"`,
    );
    return Promise.resolve();
  }
...
export function oneOffImage(
  ...
  if (globalThis.headless) {
    ...
    const image = Image.create(coords, imagePath, parent, {
      loop: false,
      animationSpeed: 0.15,
      onComplete: () => {
        Image.hide(image);
...
export function calculateCostForSingleCard(
  card: ICard,
  timesUsedSoFar: number = 0,
  caster?: IPlayer,
): CardCost {
  ...
  if (caster) {
    if (caster.mageType == 'Bloodmason') {
      ...
      if (card.id == captureSoul.id) {
        ...
        Object.keys(allUnits).includes(card.id.replace(' Miniboss', ''))
      ) {
        // Make summon spells discounted
        cardCost.manaCost = Math.floor(cardCost.manaCost * 0.7);
      }
    } else if (
      caster.mageType == 'Cleric' &&
      card.category == CardCategory.Blessings
    ) {
      cardCost.manaCost = Math.floor(cardCost.manaCost / 2);
    } else if (
      caster.mageType == 'Witch' &&
      card.category == CardCategory.Curses
    ) {
      cardCost.manaCost = Math.floor(cardCost.manaCost * 0.8);
    } else if (
      caster.mageType == 'Archer' &&
      card.id.toLowerCase().includes('arrow')
    ) {
...
export function calculateCost(
  ...
  for (let card of cards) {
    ...
    const singleCardCost = calculateCostForSingleCard(
      card,
      (casterCardUsage[card.id] || 0) + (thisCalculationUsage[card.id] || 0),
      caster,
    );
...
export function _getCardsFromIds(
  cardIds: string[],

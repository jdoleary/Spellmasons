...
export function init(unit: Unit.IUnit, underworld: Underworld, prediction: boolean) {
  // Immune units miss a turn
  ...
  const animatedFortifySprite = unit.image?.sprite.children.find(c => c.imagePath == imageName)
    || Image.addSubSprite(unit.image, imageName);
  if (animatedFortifySprite) {
    // Make it red
    (animatedFortifySprite as Sprite).tint = 0xff0000;
  }
}

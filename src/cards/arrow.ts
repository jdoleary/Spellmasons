...
import {
  ...
  getEndpointOfMagnitudeAlongVector,
  invert,
  subtract,
  Vec2,
} from '../jmath/Vec';
...
export function arrowEffect(
  multiShotCount: number,
  damageDone: number,
  onCollide?: (
    state: EffectState,
    firstTarget: Unit.IUnit,
      ...
      for (let target of targets) {
        ...
        for (let arrowNumber = 0; arrowNumber < multiShotCount; arrowNumber++) {
          ...
          if (arrowNumber > 0) {
            ...
            casterPositionAtTimeOfCast = subtract(
              casterPositionAtTimeOfCast,
              diff,
            );
...
export function findArrowPath(
  ...
  let endPoint = add(
    ...
    math.similarTriangles(
      target.x - casterPositionAtTimeOfCast.x,
      target.y - casterPositionAtTimeOfCast.y,
      math.distance(casterPositionAtTimeOfCast, target),
      10000,
    ),
  );
  let arrowShootPath = { p1: casterPositionAtTimeOfCast, p2: endPoint };
  // revise end point to stop where it hits the first wall
  const LOSResult = closestLineSegmentIntersectionWithLine(
    arrowShootPath,
    underworld.walls,
  );
  ...
  if (intersection) {
    ...
    if (LOSResult?.lineSegment) {
      const wallVector = normalizedVector(
        LOSResult.lineSegment.p2,
        LOSResult.lineSegment.p1,
      );
      if (wallVector.vector) {
        intersection = moveAlongVector(
          intersection,
          invert(wallVector.vector),
...
export function findArrowCollisions(
  casterPositionAtTimeOfCast: Vec2,
  casterId: number,
  target: Vec2,
  prediction: boolean,
  ...
  const arrowShootPath = findArrowPath(
    casterPositionAtTimeOfCast,
    target,
    underworld,
  );
  ...
  const hitTargets = (
    ...
    .filter((u) => {
      ...
      const pointAtRightAngleToArrowPath =
        findWherePointIntersectLineSegmentAtRightAngle(u, arrowShootPath);
    ...
    .sort((a, b) => {
      return (
        math.distance(a, arrowShootPath.p1) -
        math.distance(b, arrowShootPath.p1)
      );
    });

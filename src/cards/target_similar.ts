...
export function targetSimilarEffect(numberOfTargets: number) {
  return async (
    ...
    for (let i = 0; i < length; i++) {
      ...
      const potentialTargets = underworld
        ...
        .filter((t) => {
          if (isUnit(target)) {
            return (
              isUnit(t) &&
              t.unitSourceId == target.unitSourceId &&
              t.alive == target.alive &&
...
export async function animateTargetSimilar(
  circles: { pos: Vec2; newTargets: Vec2[] }[],
) {
  ...
  return raceTimeout(
    millisBetweenIterations *
      (iterations + extraIterationsToShowAnimatedCircleAtEnd + 10),
    'animatedConnect',
    new Promise<void>((resolve) => {
      // +extraIterations... gives iterations some time to show the circle at the end
      for (
        let i = 0;
        i < iterations + extraIterationsToShowAnimatedCircleAtEnd;
        i++
      ) {
        setTimeout(() => {

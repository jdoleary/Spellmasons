      ...
      for (let i = 0; i < quantity; i++) {
        for (let target of targets) {
          ...
          for (let num = 0; num < multi; num++) {
            ...
            if (num > 0) {
              const diff = subtract(casterPositionAtTimeOfCast, getEndpointOfMagnitudeAlongVector(casterPositionAtTimeOfCast, (num == 2 ? -1 : 1) * Math.PI / 2 + getAngleBetweenVec2s(state.casterPositionAtTimeOfCast, state.castLocation), 20));
